   ...
   def onTalk (self,npc,player):
      ...
      if id == CREATED and npcId == Galatea:    # start part, Galatea
         ...
         if npcId == Galatea :            # Start and End Npc Galatea related stuff
            ...
            L1 = st.getQuestItemsCount(LIST1)
            L2 = st.getQuestItemsCount(LIST2)
            L3 = st.getQuestItemsCount(LIST3)
            L4 = st.getQuestItemsCount(LIST4)
            L5 = st.getQuestItemsCount(LIST5)
            if step == 1 or step == 2 and L1==L2==L3==L4==L5==0:           # first talk to lara and option to remove list form inventory, then you can take another one
