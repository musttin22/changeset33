...
DarkElvenLongBow=[284,4825,4826,4827] #Because this stupid weapon have more m.atk from TOP B so must be removed by ID, if you REMOVE THIS then you can exchange LOW B for LOW A!!!
  ...
  def onEvent (self,event,st) :
    ...
    ## FINAL ITEM EXCHANGE SECTION
      ...
      for Item in st.getPlayer().getInventory().getItems():
        ...
        if TopBGradeWeaponData.has_key(weaponType) and not Item.isEquipped() and Item.getItemId() not in DarkElvenLongBow and itemTemplate.getCrystalType() == 0x03 and itemTemplate.getDuration() == -1 :
