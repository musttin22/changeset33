...
from l2jorion.game.model import L2World
from l2jorion.game.model.quest import State
from l2jorion.game.model.quest import QuestState
from l2jorion.game.model.quest.jython import QuestJython as JQuest
from l2jorion.game.network.serverpackets import CreatureSay
from l2jorion.game.network.serverpackets import MagicSkillUser
        ...
        if party :
            ...
            if len(PartyQuestMembers) > 0 :
                ...
                for member in PartyQuestMembers: 
                    member.giveItems(st1.getQuest().Items[8],1)
                    member.takeItems(st1.getQuest().Items[3],-1)
                    member.set("cond","16")
                    member.playSound("ItemSound.quest_middle")
        else:
            for q in Quests.keys() :
                st1 = player.getQuestState(Quests[q])
                if st1 :
