...
public enum FakePlayerManager
{
	...
	{
		return spawnPlayer(x, y, z, 81, 3, false, false, false, false);
	}
	
	public FakePlayer spawnPlayer(int x, int y, int z, int level, int classNumber, boolean pvpZoneBot, boolean walkerBot, boolean farmBot, boolean newbieBot)
	{
		FakePlayer activeChar = FakeHelpers.createRandomFakePlayer(level, classNumber, pvpZoneBot, farmBot, newbieBot);
	...
	private class loadBotsToNewbieZonestask implements Runnable
	{
		...
		public void run()
		{
			...
			LOG.info("BotEngine: Spawning (" + Config.BOTS_COUNT_NEWBIE_ZONES + ") bots to newbie zones...");
	...
	private class loadBotsToRandomTownTask implements Runnable
	{
		...
		public void run()
		{
			int LoadedBots = 0;
			int x = 0, y = 0;
			LOG.info("BotEngine: Spawning (" + Config.BOTS_COUNT_RANDOM_TOWNS + ") bots to random town...");
	...
	private class loadBotsToRandomPvpZoneTask implements Runnable
	{
		...
		public void run()
		{
			...
			LOG.info("BotEngine: Spawning (" + Config.BOTS_COUNT_PVP_ZONE + ") bots to pvp zone...");
	...
	private class loadBotsToRandomFarmZoneTask implements Runnable
	{
		...
		public void run()
		{
			...
			LOG.info("BotEngine: Spawning (" + Config.BOTS_COUNT_FARM_ZONE + ") bots to random farm zone...");
