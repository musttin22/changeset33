...
public abstract class FakePlayerAI
{
	...
	private final int toVillageIterationsOnDeath = Rnd.get(5, 20);
	...
	protected final int standNearGk = Rnd.get(10, 25);
	
	protected int iterationsForTarget = 0;
	public int limitForTarget = Rnd.get(1, 5);
	...
	protected void tryTargetRandomCreatureByTypeInRadius(Class<? extends L2Character> creatureClass, int radius)
	{
		if (_fakePlayer.getTarget() == null)
		{
			...
			List<L2Character> targets = _fakePlayer.getKnownTypeInRadius(creatureClass, radius).stream().filter(x -> //
			x.isMonster() && !x.isDead() && !x.isRaid() && !x.isRaidMinion() && !(Config.BOTS_EXCLUDE_TARGETS_LIST.contains(((L2MonsterInstance) x).getNpcId())) //
				|| x.isBot() && !x.isDead() //
				|| x.isPlayer() && !x.isBot() && !x.isDead() && GeoData.getInstance().canSeeTarget(_fakePlayer, x)).collect(Collectors.toList());
		...
		{
			if (_fakePlayer.getTarget() instanceof L2PcInstance)
			{
				if (((L2PcInstance) _fakePlayer.getTarget()).getPvpFlag() == 0) // no attack if pvp flag is gone
				{
			...
			if (_fakePlayer.getBotMode() != 3)
			{
				...
				{
					if (50 > Rnd.get(100) && _fakePlayer.getLevel() > 11)
					{
						_fakePlayer.setTargetRange(300);
	...
	public void castSpell(L2Skill skill)
	{
		if (!_fakePlayer.isCastingNow())
		{
			...
			else
			{
				...
				if (_fakePlayer.getTarget() != null)
				{
					if (_fakePlayer.getTarget().isDead())
					{
						_fakePlayer.setTarget(null);
						return;
					}
	...
	public void moveTo(int x, int y, int z)
	{
		if (!_fakePlayer.isMovementDisabled())
		{
			...
			if (_fakePlayer.isTeleportProtected())
			{
	...
	protected boolean doUnstuck(L2PcInstance activeChar)
	{
		...
		L2Skill escape = SkillTable.getInstance().getInfo(2099, 1);
		escape.setHitTime(unstuckTimer);
