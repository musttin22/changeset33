...
public class Shutdown extends Thread
{
		...
		if (this == getInstance())
		{
			...
			if (Config.SELLBUFF_SYSTEM_OFFLINE)
			{
				try
				{
					...
					OfflineBuffersData.getInstance().storeOfflineBuffersOnShutDown();
					LOG.info("Offline Buffers Table: Offline buffers stored ({}ms).", tc.getEstimatedTimeAndRestartCounter());
				}
				catch (Throwable t)
				{
					LOG.error("Error saving offline buffers:");
			...
			if ((Config.OFFLINE_TRADE_ENABLE || Config.OFFLINE_CRAFT_ENABLE) && Config.RESTORE_OFFLINERS)
			{
				try
				{
					...
					{
						OfflineTradeTable.getInstance().storeOffliners();
					}
					else
					{
						OfflineTradeTableWithBuffer.getInstance().storeOffliners();
					}
				...
				{
					LOG.error("Error saving offline shops:");
					if (Config.ENABLE_ALL_EXCEPTIONS)
					{
						t.printStackTrace();
					}
	...
	private void saveData()
	{
		...
		{
			BotsManager.getInstance().insertOrUpdateBotReportData();
			LOG.info("Bot Report Data: Saved ({}ms).", tc.getEstimatedTimeAndRestartCounter());
		}
		...
		LOG.info("Fishing Championship: Data saved ({}ms).", tc.getEstimatedTimeAndRestartCounter());
