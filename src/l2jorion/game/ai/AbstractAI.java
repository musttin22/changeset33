...
public abstract class AbstractAI implements Ctrl
{
	...
	private class FollowTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (Util.checkIfInRange(_range, _actor, followTarget, true) && followTarget.isPlayable())
				{
					if (followTarget.isAutoAttackable(_actor) && getIntention() != AI_INTENTION_ATTACK)
					{
			...
			catch (Exception e)
			{
				...
				{
					LOG.error("{}: There has been a problem running the follow task:", getClass().getSimpleName(), e);
				}
			}
		}
	}
	...
	public void moveToPawn(L2Object pawn, int offset)
	{
		if (!_actor.isMovementDisabled())
		{
			...
			{
				offset = 150; // it needs the correction for baium npc
			}
