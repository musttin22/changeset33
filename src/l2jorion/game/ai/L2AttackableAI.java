...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2Summon)
		{
			...
			if (owner != null)
			{
				...
				if (!(creature instanceof L2RaidBossInstance) && owner.isSilentMoving())
				{
					return false;
				}
	...
	private void thinkActive()
	{
				...
				if (autoAttackCondition(target)) // check aggression
				{
					...
					if (hating == 0)
					{
						switch (npc.getNpcId())
						{
							...
							case 22129:
							{
								npc.addDamageHate((L2Character) obj, 0, 0);
								break;
							}
							default:
							{
								...
								npc.addDamageHate((L2Character) obj, 0, 1);
								break;
							}
						}
					}
		...
		else if (!(npc instanceof L2ChestInstance) && !(npc instanceof L2GuardInstance) && npc.getSpawn() != null && Rnd.nextInt(RANDOM_WALK_RATE) == 0 && npc.getSpawn().isRandomWalk())
		{
			...
			if (Config.MONSTER_RETURN_DELAY > 0 && npc instanceof L2MonsterInstance && !npc.isAlikeDead() && !npc.isDead() && npc.getSpawn() != null && !npc.isInsideRadius(npc.getSpawn().getLocx(), npc.getSpawn().getLocy(), range * 2, false))
			{
				npc.returnHome();
			}
	...
	private void thinkAttack()
	{
		...
		if (Config.RON_CUSTOM) // As customer wished for hot springs mobs
		{
			switch (npc.getNpcId())
			{
				case 21316:
				...
				case 21317:
				case 21322:
					if (mostHate instanceof L2PlayableInstance)
					{
						if (npc.getTemplate().getEventQuests(Quest.QuestEventType.ON_AGGRO_RANGE_ENTER) != null)
						{
							final L2PcInstance targetPlayer = mostHate instanceof L2PcInstance ? (L2PcInstance) mostHate : ((L2Summon) mostHate).getOwner();
							for (Quest quest : npc.getTemplate().getEventQuests(Quest.QuestEventType.ON_AGGRO_RANGE_ENTER))
							{
								quest.notifyAggroRangeEnter(npc, targetPlayer, (mostHate instanceof L2Summon));
							}
						}
					}
					break;
		...
		if (!npc.isMovementDisabled() && npc.isInCombat() && (Rnd.nextInt(100) <= 15))
		{
