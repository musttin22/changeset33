...
public class L2CharacterAI extends AbstractAI
{
	...
	public boolean maybeMoveToPawn(L2Object target, int offset)
	{
		...
		if (target.isDoor())
		{
			...
			int xPoint = 0;
			int yPoint = 0;
			for (int i : door.getTemplate().getNodeX())
			{
				xPoint += i;
			}
			for (int i : door.getTemplate().getNodeY())
			{
				yPoint += i;
			}
			xPoint /= 4;
			yPoint /= 4;
			needToMove = !_actor.isInsideRadius(xPoint, yPoint, door.getTemplate().getNodeZ(), offset, false, false);
		}
		else
		{
	...
	protected void onIntentionActive()
	{
		if (getIntention() != AI_INTENTION_ACTIVE)
		{
			...
			if (_actor instanceof L2Attackable)
			{
				if (_actor.getAttackByList() != null && !_actor.getAttackByList().isEmpty())
				{
