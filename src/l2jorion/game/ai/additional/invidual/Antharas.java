...
public class Antharas extends Quest implements Runnable
{
	...
	private static final int WAITING = 1; // Antharas is spawned and someone has entered, triggering a 30 minute window for additional people to enter
	...
	private void init()
	{
		try
		{
			...
			// Antharas
			template = NpcTable.getInstance().getTemplate(29019);
			...
			// tempSpawn.setRespawnDelay(FWA_ACTIVITYTIMEOFANTHARAS * 2); Why...?
		...
		if (status == DEAD)
		{
			ThreadPoolManager.getInstance().scheduleGeneral(new UnlockAntharas(29019), respawnTime);
		}
		...
		else if (status == FIGHTING)
		{
			...
			{
				Announcements.getInstance().announceWithServerName("The Grand boss " + _antharas.getName() + " spawned in the world.");
			}
		}
	}
	...
	public void customSpawnAntharas()
	{
		if (_antharas == null)
		{
			if (_monsterSpawnTask == null)
			{
				synchronized (this)
				{
					...
					{
						GrandBossManager.getInstance().setBossStatus(29019, WAITING);
						_monsterSpawnTask = ThreadPoolManager.getInstance().scheduleGeneral(new AntharasSpawn(1), 1000);
					}
				}
			}
		}
	...
	// Move at random after Antharas appears
	...
	public String onKill(L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		if (npc.getNpcId() == 29019)
		{
			...
			if (!npc.getSpawn().is_customBossInstance())
			{
				...
				long respawnTime = (Config.ANTHARAS_RESP_FIRST * 3600000);
				int randomTime = (Config.ANTHARAS_RESP_SECOND * 3600000);
	...
	{
		protected static final Antharas INSTANCE = new Antharas(-1, "antharas", "ai");
	}
