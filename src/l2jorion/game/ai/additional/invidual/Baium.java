...
public class Baium extends Quest implements Runnable
{
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		{
			Announcements.getInstance().announceWithServerName("The Grand boss Baium Stone will be spawned in " + (Config.SPAWN_ANNOUNCE_MESSAGE_TIME / 60000) + " min.");
		}
	...
	public String onTalk(L2NpcInstance npc, L2PcInstance player)
	{
		...
		if (npcId == STONE_BAIUM && status == ASLEEP)
		{
			if (_zone.isPlayerAllowed(player))
			{
				...
				{
					Announcements.getInstance().announceWithServerName("The Grand boss " + baium.getName() + " spawned in the world.");
				}
		...
		else if (npcId == ANGELIC_VORTEX)
		{
			...
			if ((GrandBossManager.getInstance().getBossStatus(LIVE_BAIUM) == ASLEEP) && (player.getQuestState("baium").getQuestItemsCount(4295) > 0 || has_special_item)) // bloody fabric
			{
				if (!has_special_item)
				{
	...
	public String onKill(L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		if (npc.getNpcId() == LIVE_BAIUM)
		{
			...
			if (!npc.getSpawn().is_customBossInstance())
			{
				...
				long respawnTime = (Config.BAIUM_RESP_FIRST * 3600000);
				int randomTime = (Config.BAIUM_RESP_SECOND * 3600000);
