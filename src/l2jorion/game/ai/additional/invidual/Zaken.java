...
public class Zaken extends Quest implements Runnable
{
	...
	// Zaken loc
	private static int zakenLocX = 55312;
	private static int zakenLocY = 219168;
	private static int zakenLocZ = -3223;
	...
	public Zaken(int questId, String name, String descr)
	{
		...
		if (status == DEAD)
		{
			long respawnTime = (info.getLong("respawn_time") - System.currentTimeMillis());
			if (respawnTime > 0)
			{
			...
			{
				L2GrandBossInstance zaken = (L2GrandBossInstance) addSpawn(ZAKEN, zakenLocX, zakenLocY, zakenLocZ, 0, false, 0);
	...
	public void spawnBoss(L2GrandBossInstance npc)
	{
		if (npc == null)
		{
			LOG.error("Zaken AI failed to load, missing Zaken in grandboss_data.sql");
			return;
		}
		...
		if (_zone == null)
		{
			LOG.error("Zaken AI failed to load, missing zone for Zaken");
			return;
		}
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("zaken_unlock"))
		{
			...
			{
				Announcements.getInstance().announceWithServerName("The Grand boss " + zaken.getName() + " spawned in the world.");
			}
		}
		else if (event.equalsIgnoreCase("custom_unlock_message"))
		{
			Announcements.getInstance().announceWithServerName("The Raid boss Zaken will be spawned in " + (Config.SPAWN_ANNOUNCE_MESSAGE_TIME / 60000) + " min.");
		}
	...
	public String onKill(L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		...
		if (npcId == ZAKEN)
		{
			if (!npc.getSpawn().is_customBossInstance())
			{
				...
				long respawnTime = (Config.ZAKEN_RESP_FIRST * 3600000);
				int randomTime = (Config.ZAKEN_RESP_SECOND * 3600000);
	...
	public static void openCloseGates()
	{
		...
		try
		{
			...
			DoorTable.getInstance().getDoor(21240006).openMe();
			LOG.info("Zaken: Gates opened at " + h + ":" + m);
			ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
			{
				@Override
				public void run()
				{
					try
					{
						...
						LOG.info("Zaken: Gates closed at " + GameTimeController.getInstance().getGameHour() + ":" + GameTimeController.getInstance().getGameMinute());
