...
public class AutofarmPlayerRoutine
{
	...
	private void usePCBangPoints()
	{
		if (Config.AUTOFARM_FOR_PCBP)
		{
			// PC Bang points amount
			int points = Config.AUTOFARM_FOR_PCBP_PER_TIME_AMOUNT;
			// Time in minutes
			int time = Math.abs(Config.AUTOFARM_FOR_PCBP_TIME * 60);
			
			if (player.getPcBangScore() < points)
			{
				player.sendPacket(new SystemMessage(SystemMessageId.NOT_ENOUGH_ITEMS));
				AutofarmManager.INSTANCE.onLackOfItem(player);
				return;
			}
			
			if (firstAction)
			{
				player.reducePcBangScore(points);
				player.sendMessage("Paid for autofarming: " + points + " PC Bang Point" + (points > 1 ? "s" : ""));
				player.sendPacket(new ExShowScreenMessage("Paid for autofarming: " + points + " PC Bang Point" + (points > 1 ? "s" : ""), 2000, 2, false));
				firstAction = false;
			}
	...
	public void targetEligibleCreature()
	{
		if (player.getTarget() != null)
		{
			if (!player.getTarget().isDead() && !player.isMoving() && !GeoData.getInstance().canSeeTarget(player, player.getTarget()))
			{
				player.getAI().moveToPawn(player.getTarget(), 0);
				return;
			}
			
			if (!player.getTarget().isDead() && GeoData.getInstance().canSeeTarget(player, player.getTarget()))
			{
				return;
			}
			
			player.setAutoFarmEngineRadius(200);
		...
		switch (player.getAutoFarmMode().toLowerCase())
		{
			case "random":
			{
				targets = getKnownMonstersInRadius(player, player.getAutoFarmEngineRadius());
				break;
			}
			case "radius":
			{
				targets = getKnownTypeInRadius(player, player.getAutoFarmEngineRadius());
				break;
			}
		}
		...
		{
			if (player.getAutoFarmEngineRadius() < player.getAutoFarmRadius())
			{
				player.setAutoFarmEngineRadius(player.getAutoFarmEngineRadius() + 200);
			}
			return;
		}
		
		L2Character closestTarget = targets.stream().min((o1, o2) -> (int) Util.calculateDistance(o1, o2, false)).get();
	...
	public final List<L2MonsterInstance> getKnownMonstersInRadius(L2PcInstance player, int radius)
	{
		...
		for (L2WorldRegion reg : region.getSurroundingRegions())
		{
			...
			{
				if (!obj.isMonster() || obj == player || obj.isDead() || !Util.checkIfInRange(radius, player, obj, true))
				{
					continue;
				}
				
				if (!GeoData.getInstance().canMove(player.getX(), player.getY(), player.getZ(), obj.getX(), obj.getY(), obj.getZ(), 0))
				{
					continue;
				}
	...
	public final List<L2MonsterInstance> getKnownTypeInRadius(L2PcInstance player, int radius)
	{
		final L2WorldRegion region = player.getWorldRegion();
		if (region == null)
		{
		...
		for (L2WorldRegion reg : region.getSurroundingRegions())
		{
			for (L2Object obj : reg.getVisibleObjects().values())
			{
				if (!obj.isMonster() || obj == player || obj.isDead() || !Util.checkIfInRange(radius, obj, player, true))
				{
					continue;
				}
