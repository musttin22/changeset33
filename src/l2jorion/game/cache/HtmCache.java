...
public class HtmCache
{
	...
	public void reload(File f)
	{
		...
		{
			LOG.info("Html cache starts...");
			parseDir(f);
			LOG.info(getClass().getSimpleName() + ": " + String.format("%.3f", getMemoryUsage()) + " megabytes on " + getLoadedFiles() + " files loaded");
		}
		else
		{
			_cache.clear();
			_loadedFiles = 0;
			_bytesBuffLen = 0;
			LOG.info(getClass().getSimpleName() + ": Running cache");
		}
	}
	...
	{
		return getHtm(path, null, 0);
	}
	
	public String getHtm(String path, L2PcInstance player, int type)
	{
