...
public class MailBBSManager extends BaseBBSManager implements ICommunityBoardHandler
{
	...
	private void initId()
	{
		...
		{
			LOG.error("Couldn't find the last mail id.", e);
		}
		finally
		{
			CloseUtil.close(con);
	...
	private Set<Mail> getPlayerMails(int objectId)
	{
		...
		if (mails == null)
		{
			...
			{
				LOG.error("Couldn't load mail for player id : {}.", e, objectId);
			}
			finally
			{
				CloseUtil.close(con);
	...
	private Mail getMail(L2PcInstance player, int mailId)
	{
	...
	public int checkUnreadMail(L2PcInstance player)
	{
		return (int) getPlayerMails(player.getObjectId()).stream().filter(l -> l.unread).count();
	}
	
	private void showMailList(L2PcInstance player, int page, MailType type)
	{
		showMailList(player, page, type, "", "");
	}
	
	private void showMailList(L2PcInstance player, int page, MailType type, String sType, String search)
	{
		Set<Mail> mails;
		...
		StringUtil.append(sb, "<td><button action=\"bypass _bbsmail;", type, ";", (page == 1 ? page : page - 1), fullSearch, "\" back=\"l2ui_ch3.prev1_down\" fore=\"l2ui_ch3.prev1\" width=16 height=16></td>");
		...
		StringUtil.append(sb, "<td><button action=\"bypass _bbsmail;", type, ";", (page == maxpage ? page : page + 1), fullSearch, "\" back=\"l2ui_ch3.next1_down\" fore=\"l2ui_ch3.next1\" width=16 height=16 ></td>");
	...
	private void showMailView(L2PcInstance player, Mail mail)
	{
		if (mail == null)
		{
			showMailList(player, 1, MailType.INBOX);
	...
	private static void showWriteView(L2PcInstance player)
	{
		String content = HtmCache.getInstance().getHtm(CB_PATH + "mail/mail-write.htm");
		separateAndSend(content, player);
	}
	
	private static void showWriteView(L2PcInstance player, Mail mail)
	{
	...
	public void sendMail(String recipients, String subject, String message, L2PcInstance player)
	{
		sendMail(recipients, subject, message, player, false);
	}
	
	public void sendMail(String recipients, String subject, String message, L2PcInstance player, boolean rewardMail)
	{
		// Current time.
		...
		if (recipientNames.length > 5 && !player.isGM() && !rewardMail)
		{
		...
		try
		{
			...
			for (String recipientName : recipientNames)
			{
				...
				final int recipientId = CharNameTable.getInstance().getIdByName(recipientName);
				
				if (recipientId <= 0 || (recipientId == player.getObjectId() && !rewardMail))
				{
	...
	private static boolean isBlocked(L2PcInstance player, int objectId)
	{
		for (L2PcInstance playerToTest : L2World.getInstance().getAllPlayers().values())
		{
	...
	private void deleteMail(L2PcInstance player, int mailId)
	{
		for (Mail mail : getPlayerMails(player.getObjectId()))
		{
	...
	{
		String name = CharNameTable.getInstance().getNameById(objectId);
	...
	private void showLastForum(L2PcInstance player)
	{
