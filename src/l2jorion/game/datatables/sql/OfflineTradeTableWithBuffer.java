...
public class OfflineTradeTableWithBuffer
{
	private static Logger LOG = LoggerFactory.getLogger(OfflineTradeTableWithBuffer.class);
	
	// SQL DEFINITIONS
	private static final String SAVE_OFFLINE_STATUS = "INSERT INTO character_offline_trade (`charId`,`name`,`time`,`type`,`title`,`buffer`) VALUES (?,?,?,?,?,?)";
	private static final String SAVE_ITEMS = "INSERT INTO character_offline_trade_items (`charId`,`item`,`count`,`price`,`enchant`,`buffId`,`sellItemId`) VALUES (?,?,?,?,?,?,?)";
	...
	public static final OfflineTradeTableWithBuffer getInstance()
	{
						...
						switch (pc.getPrivateStoreType())
						{
							case L2PcInstance.STORE_PRIVATE_BUY:
								...
								for (TradeItem i : pc.getBuyList().getItems())
								{
									...
									stm_items.setLong(6, 0);
									stm_items.setInt(7, pc.getBuyList().getSellBuyItemId());
							...
							case L2PcInstance.STORE_PRIVATE_PACKAGE_SELL:
								...
								for (TradeItem i : pc.getSellList().getItems())
								{
									...
									if (pc.getSellList().isBuffer())
									{
										stm_items.setLong(6, i.getId());
										stm_items.setInt(7, pc.getSellList().getSellBuyItemId());
									}
									else
									{
										stm_items.setLong(6, 0);
	...
	public void restoreOfflineTraders()
	{
		...
		try
		{
					...
					switch (type)
					{
						case L2PcInstance.STORE_PRIVATE_BUY:
							while (items.next())
							{
								...
								if (items.getInt(7) > 0)
								{
									player.getBuyList().setSellBuyItemId(items.getInt(7));
								}
							}
						...
						case L2PcInstance.STORE_PRIVATE_PACKAGE_SELL:
							while (items.next())
							{
								if (buffer == 1)
								{
									player.getSellList().setBuffer(true);
									player.getSellList().setSellBuyItemId(items.getInt(7));
									
									L2ItemInstance item = new L2ItemInstance(items.getInt(2), items.getInt(6));
									item.setEnchantLevel(items.getInt(5));
									L2World.getInstance().storeObject(item);
									
									player.getSellList().addBuff(items.getInt(2), items.getInt(3), items.getInt(4));
								}
								else
								{
									if (items.getInt(7) > 0)
			...
			if (nTraders > 0)
			{
				Util.printSection("Offline traders");
				LOG.info(getClass().getSimpleName() + ": Loaded " + nTraders + " offline trader" + (nTraders > 1 ? "s" : ""));
			}
		}
		catch (Exception e)
		{
	...
	public void storeOffliner(L2PcInstance pc)
	{
	...
	{
		protected static final OfflineTradeTableWithBuffer _instance = new OfflineTradeTableWithBuffer();
	}
}