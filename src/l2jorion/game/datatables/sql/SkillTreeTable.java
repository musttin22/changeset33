		...
		for (final L2EnchantSkillLearn skillLearn : skills)
		{
			...
			if (skill != null && ((skill.getLevel() == SkillTable.getInstance().getMaxLevel(skill.getId()) && (skillLearn.getLevel() == 101 || skillLearn.getLevel() == 141)) || (skill.getLevel() == skillLearn.getLevel() - 1)))
			{
				if (Config.ALPHA_CUSTOM || Config.DESIMKE_CUSTOM || Config.TADUSE_CUSTOM)
				{
					if (player.isPlayer())
					{
						if (((L2PcInstance) player).isCustomEnchantSkill())
						{
							int limit = 0;
							
							if (skill.getLevel() > 100 && skill.getLevel() < 131)
							{
								limit = (100 + PowerPackConfig.GMSHOP_SKILL_ENCHNANT_LIMIT);
								if (skill.getLevel() >= limit)
								{
									continue;
								}
							}
							else if (skill.getLevel() > 140 && skill.getLevel() < 171)
							{
								limit = (140 + PowerPackConfig.GMSHOP_SKILL_ENCHNANT_LIMIT);
								if (skill.getLevel() >= limit)
								{
									continue;
								}
