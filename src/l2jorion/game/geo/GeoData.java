...
public class GeoData
{
	...
	private static final int ELEVATED_SEE_OVER_DISTANCE = 0;
	...
	public Location moveCheck(int x, int y, int z, int tx, int ty, int tz, int instanceId)
	{
		...
		if (hasGeoPos(prevX, prevY) && (prevZ != tz)) // Removed. It makes a bug -> && Math.abs(prevZ - tz) > 1000)
		{
			return null; // new Location(x, y, z);
		}
