...
public class GeoPathFinding extends PathFinding
{
		...
		try
		{
			...
			Files.lines(Paths.get("./data/pathnodes/pn_index.txt"), StandardCharsets.UTF_8)
				...
				.forEach(line -> {
					...
					{
						LOG.warn("Invalid pathnode entry: '" + line + "', must be in format 'XX_YY', where X and Y - integers");
						return;
					}
	...
	private void LoadPathNodeFile(byte rx, byte ry)
	{
		...
		{
			LOG.error("Failed to Load PathNode File: " + file.getAbsolutePath() + " : " + e.getMessage(), e);
		}
	}
