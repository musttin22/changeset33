	...
	private void restoreChatData()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				PreparedStatement statement2 = con.prepareStatement("SELECT * FROM auto_chat_text WHERE groupId=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
			...
			if (Config.DEBUG)
			{
				LOG.debug("AutoChatHandler: Loaded " + numLoaded + " chat groups from the database");
			}
		}
		catch (final Exception e)
		{
			LOG.warn("AutoChatHandler: Could not restore chat data: " + e);
