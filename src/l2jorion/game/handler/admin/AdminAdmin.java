...
import l2jorion.ConfigParameters;
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		switch (comm)
		{
			...
			case admin_admin5:
				...
				return true;
			case admin_gmlist:
				FastMap.Entry<L2PcInstance, Boolean> gm = GmListTable.getInstance().getGmListTable().getEntry(activeChar);
				if (gm != null)
				{
					if (gm.getValue())
					{
			...
			case admin_set:
				...
				try
				{
					cmd = st.nextToken().split("_");
				}
				catch (NoSuchElementException e)
				{
					// Just catch unnecessary error
				}
				...
				if (cmd != null && cmd.length > 1)
				{
					if (st.hasMoreTokens())
					{
						...
						if (parameter.length > 1)
						{
							...
							if (ConfigParameters.setParameterValue(pName, pValue))
							{
								activeChar.sendMessage("parameter " + pName + " successfully set to " + pValue);
							}
							else
							{
