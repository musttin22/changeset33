...
public class AdminBan implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_ban_acc"))
		{
			...
			{
				activeChar.sendMessage("Usage: //ban_acc <account_name>(if none, target char's account gets banned <reason>)");
				return false;
			}
			...
			else if (targetPlayer != null && targetPlayer.isInOfflineMode())
			{
				...
				Announcements.getInstance().punishmentAnnounceToAll(text);
			}
