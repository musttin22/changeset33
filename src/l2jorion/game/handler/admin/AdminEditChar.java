...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		switch (comm)
		{
			...
			case admin_setcolorname:
			{
				...
				{
					activeChar.sendMessage("Usage: //setcolorname <new_color>");
					return false;
				}
			...
			case admin_setcolortitle:
			{
				...
				{
					activeChar.sendMessage("Usage: //setcolortitle <new_color>");
					return false;
				}
			...
			case admin_phantom_give_item:
			{
				...
				item = ((L2PcInstance) activeChar.getTarget()).getInventory().addItem("Item", Integer.parseInt(itemId), 1, ((L2PcInstance) activeChar.getTarget()), null);
				
				if (item != null)
				{
					if (((L2PcInstance) activeChar.getTarget()) instanceof FakePlayer)
					{
						((L2PcInstance) activeChar.getTarget()).getAutoSoulShots().clear();
					}
					((L2PcInstance) activeChar.getTarget()).getInventory().equipItemAndRecord(item);
					((L2PcInstance) activeChar.getTarget()).getInventory().reloadEquippedItems();
				}
	...
	{// Page limit
		...
		htm.replace("%bots%", isShowBots() ? "<button value=\"\" action=\"bypass admin_show_characters_opion 3\" width=15 height=15 back=\"L2UI.CheckBox\" fore=\"L2UI.CheckBox_checked\">" : "<button value=\"\" action=\"bypass admin_show_characters_opion 3\" width=15 height=15 back=\"L2UI.CheckBox_checked\" fore=\"L2UI.CheckBox\">");
		htm.replace("%offliners%", isShowOffliners() ? "<button value=\"\" action=\"bypass admin_show_characters_opion 4\" width=15 height=15 back=\"L2UI.CheckBox\" fore=\"L2UI.CheckBox_checked\">" : "<button value=\"\" action=\"bypass admin_show_characters_opion 4\" width=15 height=15 back=\"L2UI.CheckBox_checked\" fore=\"L2UI.CheckBox\">");
		htm.replace("%onliners%", isShowPlayers() ? "<button value=\"\" action=\"bypass admin_show_characters_opion 5\" width=15 height=15 back=\"L2UI.CheckBox\" fore=\"L2UI.CheckBox_checked\">" : "<button value=\"\" action=\"bypass admin_show_characters_opion 5\" width=15 height=15 back=\"L2UI.CheckBox_checked\" fore=\"L2UI.CheckBox\">");
	...
	public static void gatherCharacterInfo(final L2PcInstance activeChar, final L2PcInstance player, final String filename)
	{
		...
		adminReply.replace("%hpbar%", Util.generateBar(150, 12, (long) player.getCurrentHp(), player.getMaxHp(), "HP"));
		adminReply.replace("%cpbar%", Util.generateBar(150, 12, (long) player.getCurrentCp(), player.getMaxCp(), "CP"));
		adminReply.replace("%mpbar%", Util.generateBar(150, 12, (long) player.getCurrentMp(), player.getMaxMp(), "MP"));
		adminReply.replace("%expbar%", Util.generateBar(150, 12, player.getExp() + 1, ExperienceData.getInstance().getExpForLevel(player.getLevel() + 1), "EXP"));
		
		adminReply.replace("%xp%", String.valueOf(player.getExp() + 1));
		adminReply.replace("%xpforlevel%", String.valueOf(ExperienceData.getInstance().getExpForLevel(player.getLevel() + 1)));
		
		adminReply.replace("%sp%", String.valueOf(player.getSp()));
		
		adminReply.replace("%title%", String.valueOf(player.getTitle()));
		...
		{
			FakePlayer bot = (FakePlayer) player;
			
			switch (bot.getBotMode())
			{
				case 1:
					type = "Newbie";
					break;
			...
			adminReply.replace("%botActionId%", String.valueOf(bot.getActionId()));
			adminReply.replace("%botLocId%", String.valueOf(bot.getLocId()));
	...
	private void showCharacterInfo(final L2PcInstance activeChar, L2PcInstance player)
	{
		...
		if (player.isBot())
		{
			gatherCharacterInfo(activeChar, player, "botinfo.htm");
		}
		else
		{
