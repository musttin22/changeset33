	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		else if (actualCommand.equalsIgnoreCase("admin_set_level") || actualCommand.equalsIgnoreCase("admin_setlevel"))
		{
			try
			{
				...
				{
					activeChar.sendPacket(new SystemMessage(SystemMessageId.INCORRECT_TARGET)); // incorrect
					return false;
				}
				
				L2PlayableInstance targetPlayer = (L2PlayableInstance) targetChar;
				...
				{
					long pXp = targetPlayer.getStat().getExp();
					long tXp = ExperienceData.getInstance().getExpForLevel(lvl);
					
					if (targetPlayer instanceof L2PetInstance)
					{
						pXp = targetPlayer.getStat().getExp();
						tXp = L2PetDataTable.getInstance().getPetData(((L2PetInstance) targetPlayer).getNpcId(), lvl).getPetMaxExp();
					}
