...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_spawn") || command.startsWith("admin_spawn_monster"))
		{
			...
			{
				activeChar.sendMessage("Usage: //spawn <npc_id>");
				activeChar.sendMessage("Example: //spawn 55555");
				activeChar.sendMessage("or");
				activeChar.sendMessage("Usage: //spawn <npc_id> <amount> <respawn_time>");
				activeChar.sendMessage("Example: //spawn 55555 1 60");
				AdminHelpPage.showHelpPage(activeChar, "spawns.htm");
			}
			catch (Exception e)
	...
	private void showMonsters(L2PcInstance activeChar, int level, int from)
	{
		...
		{
			String txt = "<a action=\"bypass -h admin_spawn_monster " + mobs[i].npcId + "\">" + mobs[i].name + " " + (mobs[i].aggroRange > 0 ? "<font color=ff0000>*</font>" : "") + "</a><br1>";
