...
public class CustomAugmentationSystem implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		final L2ItemInstance targetItem = activeChar.getActiveWeaponInstance();
		final L2ItemInstance refinerItem = activeChar.getInventory().getItemByItemId(item.getItemId());
		...
		activeChar.setTargetWeapon(targetItem);
		activeChar.setRefinerItem(refinerItem);
		
		if (targetItem.isAugmented())
		{
			sendDlgMessage(targetItem.getItemName() + " is already augmented. If you want to remove it - click on OK", activeChar);
			return;
		}
		
		sendDlgMessage(targetItem.getItemName() + " will be augmented", activeChar);
	}
	
	private void sendDlgMessage(final String text, final L2PcInstance player)
	{
	...
	public void onDlgAnswer(final L2PcInstance activeChar, final int answer)
	{
		if (!activeChar.getTargetWeapon().isAugmented())
		{
			// Augmentation
			if (answer == 1)
			{
				// unequip item
				if (activeChar.getTargetWeapon().isEquipped())
				{
					activeChar.disarmWeapons();
				}
				
				if (TryAugmentItem(activeChar, activeChar.getTargetWeapon(), activeChar.getRefinerItem()))
				{
					final int stat12 = 0x0000FFFF & activeChar.getTargetWeapon().getAugmentation().getAugmentationId();
					final int stat34 = activeChar.getTargetWeapon().getAugmentation().getAugmentationId() >> 16;
		...
		if (answer == 1)
		{
			...
			activeChar.getTargetWeapon().getAugmentation().removeBoni(activeChar);
			activeChar.getTargetWeapon().removeAugmentation(activeChar);
	...
	private boolean TryAugmentItem(final L2PcInstance player, final L2ItemInstance targetItem, final L2ItemInstance refinerItem)
	{
		...
		if (itemGrade < L2Item.CRYSTAL_C || itemType != L2Item.TYPE2_WEAPON || !Config.ALPHA_CUSTOM && !targetItem.isDestroyable())
		{
			return false;
		}
