...
public class ExtractableItems implements IItemHandler
{
	...
	public void doExtract(final L2PlayableInstance playable, final L2ItemInstance item, int count)
	{
		...
		while (count-- > 0)
		{
			...
			int createItemID = 0, createAmount = 0, mainItemChance = 0;
			...
			for (final L2ExtractableProductItem expi : exitem.getProductItems())
			{
				...
				mainItemChance = exitem.getMainItemChance();
				
				if (mainItemChance >= rndNum && mainItemChance > 0)
				{
			...
			if (createItemID == 0 && mainItemChance == 0)
			{
			...
			{
				// PC bang points
				if (createItemID == 65436)
				{
					activeChar.addPcBangScore(createAmount);
					
					SystemMessage sm = new SystemMessage(SystemMessageId.YOU_RECEVIED_$51_GLASSES_PC);
					sm.addNumber(createAmount);
					activeChar.sendPacket(sm);
					
					activeChar.updatePcBangWnd(createAmount, true, false);
				}
				else if (mainItemChance != 0)
				{
					for (L2ExtractableProductItem list : exitem.getProductItems())
					{
						activeChar.addItem("Extract", list.getId(), list.getAmmount(), item, false);
						
						SystemMessage sm;
						if (createAmount > 1)
						{
							sm = new SystemMessage(SystemMessageId.EARNED_S2_S1_S);
							sm.addItemName(list.getId());
							sm.addNumber(list.getAmmount());
						}
						else
						{
