...
{
	private static Logger LOG = LoggerFactory.getLogger(Disablers.class);
				...
				case CANCEL:
				{
					...
					if (Formulas.getInstance().calcSkillSuccess(activeChar, target, skill, ss, sps, bss))
					{
						...
						for (L2Effect e : effects)
						{
							...
							if (Config.L2LIMIT_CUSTOM || Config.RON_CUSTOM || Config.XSAGAZ_CUSTOM)
							{
						...
						if (Config.L2LIMIT_CUSTOM || Config.RON_CUSTOM || Config.XSAGAZ_CUSTOM)
						{
							...
							if (target instanceof L2PcInstance)
							{
								...
								{
									ThreadPoolManager.getInstance().scheduleGeneral(new CancelReturn((L2PcInstance) target, ((L2PcInstance) target).getCancelledBuffs()), time * 1000);
								}
							}
						}
					}
	...
	private void negateEffect(L2Character target, SkillType type, double power, int skillId)
	{
		...
		for (L2Effect e : effects)
		{
			...
			else if ((e.getSkill().getSkillType() == type && e.getSkill().getLevel() <= power) || (e.getSkill().getEffectType() != null && e.getSkill().getEffectType() == type && e.getSkill().getEffectLvl() <= power))
			{
				if (skillId != 0)
				{
					if (skillId == e.getSkill().getId())
