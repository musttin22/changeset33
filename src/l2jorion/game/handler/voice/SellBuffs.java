...
public class SellBuffs implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, L2PcInstance activeChar, String parameter)
	{
		...
		if (command.equals(_voicedCommands[0]))
		{
			...
			{
				activeChar.sendMessage("That's not selling zone.");
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
			...
			tb.append("<p><edit var=\"price\" width=75 height=15 type=number length=11></p>");
			tb.append("<button value=\"Confirm\" action=\"bypass -h actr $price\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\">");
			tb.append("</center><br>");
			tb.append("</body></html>");
		...
		else if (command.equals(_voicedCommands[1]))
		{
			if (activeChar.isSellingBuff())
			{
				activeChar.setSellingBuff(false);
				activeChar.standUp();
