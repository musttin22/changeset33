...
public abstract class IdFactory
{
	...
	private void cleanUpDB()
	{
		...
		try
		{
			...
			// Delete bots data
			if (Config.BOTS_SYSTEM && Config.BOTS_DELETE_DATA)
			{
				cleanCount += stmt.executeUpdate("DELETE FROM characters WHERE account_name = '" + Config.BOTS_ACCOUNT + "';");
				if (cleanCount > 0)
				{
					LOG.info("BotEngine: Deleted " + cleanCount + " bots data from database");
				}
			}
