...
public class CrownManager
{
	...
	public void checkCrowns(final L2PcInstance activeChar)
	{
		...
		if (crownId > 0)
		{
			...
			{
				if (Config.XSAGAZ_CUSTOM && Config.CASTLE_REWARD_ID > 0)
				{
					activeChar.addItem("Gift", Config.CASTLE_REWARD_ID, Config.CASTLE_REWARD_AMOUNT, null, true);
				}
				...
				{
					L2Skill bonusSkill = activeChar.getSkill(9005);
					if (bonusSkill == null)
					{
						activeChar.addSkill(SkillTable.getInstance().getInfo(9005, 1), true);
		...
		for (final L2ItemInstance item : activeChar.getInventory().getItems())
		{
			if (CrownTable.getCrownList().contains(item.getItemId()))
			{
				...
				if (Config.L2UNLIMITED_CUSTOM)
				{
					...
					if (bonusSkill != null)
					{
						activeChar.removeSkill(bonusSkill);
						activeChar.sendSkillList();
					}
				}
