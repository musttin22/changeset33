...
import l2jorion.game.datatables.sql.ServerMemoTable;
...
{
	private static Logger LOG = LoggerFactory.getLogger(FishingChampionshipManager.class);
	...
	protected FishingChampionshipManager()
	{
		...
		{
			ThreadPoolManager.getInstance().scheduleGeneral(this::finishChamp, _endDate - System.currentTimeMillis());
		}
	}
	...
	public void showMidResult(L2PcInstance player)
	{
		...
		if (_needRefresh)
		{
			...
			refreshResult();
			ThreadPoolManager.getInstance().scheduleGeneral(() -> _needRefresh = true, 60000);
			return;
		}
		...
		html.replace("%prizeItem%", L2Item.getItemNameById(Config.FISH_CHAMPIONSHIP_REWARD_ITEM));
	...
	public void showChampScreen(L2PcInstance player, int objectId)
	{
