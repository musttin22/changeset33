...
public class GrandBossManager
{
	...
	private static final String UPDATE_GRAND_BOSS_DATA3 = "UPDATE grandboss_data SET loc_x = ?, loc_y = ?, loc_z = ?, heading = ?, respawn_time = ?, currentHP = ?, currentMP = ?, status = ? WHERE boss_id = ?";
	...
	public void init()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				info.set("next_respawn", rset.getString("next_respawn"));
	...
	private void storeToDb()
	{
		...
		try
		{
			...
			for (Integer bossId : _storedInfo.keySet())
			{
				...
				{
					PreparedStatement update1 = con.prepareStatement(UPDATE_GRAND_BOSS_DATA3);
