...
public class Achievement
{
	...
	public void increase(AchType type, int count, boolean increase, boolean reset, boolean daily, int id)
	{
		...
		/*
		 * AchievementHolder achDaily = AchievementManager.getInstance().getAchievements().get(type).get(0); if (daily) { if (achDaily.getNpcId() != 0 && !(String.valueOf(achDaily.getNpcId()).contains(String.valueOf(id)))) { return; } if (achDaily.getItemId() != 0 &&
		 * !(String.valueOf(achDaily.getItemId()).contains(String.valueOf(id)))) { return; } if (type == AchType.DAILY_ONLINE) { if (achDaily.getRequired() != getCount(AchType.DAILY_ONLINE)) { return; } } }
		 */
		
		if (_data.containsKey(type))
		{
			final AchievementHolder achData = AchievementManager.getInstance().getAchievements().get(type).get(_data.get(type).getId() - 1);
			if (achData.getNpcId() != 0 && !(String.valueOf(achData.getNpcId()).contains(String.valueOf(id))))
			{
				return;
			}
			
			if (achData.getItemId() != 0 && !(String.valueOf(achData.getItemId()).contains(String.valueOf(id))))
			{
				return;
			}
		}
		...
		{
			if (ach.getRequired() != getCount(AchType.DAILY_ONLINE))
			{
				return;
			}
		}
		
		if (ach.getLevel() == _data.get(type).getId() && ach.getNpcId() != 0 && (String.valueOf(ach.getNpcId()).contains(String.valueOf(id))))
		{
			_player.sendPacket(new PlaySound(0, "ItemSound.quest_itemget", 1, _player.getObjectId(), _player.getX(), _player.getY(), _player.getZ()));
			_player.sendMessage("......................[ Achievement: " + ach.getName() + " ]......................");
			_player.sendMessage("Killed: " + _data.get(type).getValue() + " / " + ach.getRequired() + "");
			_player.sendMessage(".................................................................................");
		}
		...
		if ((daily || ach.getLevel() == _data.get(type).getId()) && ach.getRequired() <= _data.get(type).getValue())
		{
			...
			String CongratsText = "Congratulations! Stage " + ach.getLevel() + "  of " + ach.getName() + " achievement completed.";
			_player.sendMessage(CongratsText);
			_player.sendPacket(new ExShowScreenMessage(CongratsText, 5000, 0x02, false));
			_player.sendPacket(new PlaySound("ItemSound.quest_fanfare_1"));
	...
	public final boolean getType(AchType type)
	{
