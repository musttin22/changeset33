...
public class CursedWeapon
{
	...
	public void endOfLife()
	{
		if (_isActivated)
		{
			...
			else
			{
				if (Config.RON_CUSTOM)
				{
					L2PcInstance player = L2PcInstance.load(_playerId);
					if (player != null)
					{
	...
	public void reActivate()
	{
		...
		else
		{
			...
			_dailyTimer = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new DailyTimer(), 60000L, 60000L);
		}
	}
	...
	private class DailyTimer implements Runnable
	{
		...
		public void run()
		{
			...
			if (_player != null && _player.isOnline() == 1 && _timer % 15 == 0)
			{
				_timer = 0;
	...
	public void cursedOnLogin()
	{
		...
		sm.addZoneName(_player.getX(), _player.getY(), _player.getZ()); // Region Name
		sm.addItemName(_player.getCursedWeaponEquipedId());
		CursedWeaponsManager.announce(sm);
