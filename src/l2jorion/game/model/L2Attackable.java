...
public class L2Attackable extends L2NpcInstance
{
	...
	public void reduceCurrentHp(double damage, L2Character attacker, boolean awake)
	{
		...
		if (this instanceof L2MonsterInstance)
		{
			...
			if (this instanceof L2MinionInstance && getNpcId() != 27189)
			{
			...
			synchronized (getAggroList())
			{
				...
				for (AggroInfo info : getAggroListRP().values())
				{
					...
					if (damage > 1)
					{
						...
						{
							if (ddealer instanceof L2PetInstance)
							{
								maxDealer = null;
							}
							else
				...
				for (RewardInfo reward : rewards.values())
				{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							if (attacker instanceof L2PcInstance)
							{
								...
								if (Config.L2LIMIT_CUSTOM)
								{
									if (!isRaid() && !isMinion())
									{
										if (((L2PcInstance) attacker).isIncreasmentRatesHigh())
										{
											exp *= 1.50;
											sp *= 1.50;
										}
										else if (((L2PcInstance) attacker).isIncreasmentRatesLow())
										{
											exp *= 1.25;
											sp *= 1.25;
										}
					...
					else
					{
						...
						for (L2PcInstance pl : groupMembers)
						{
							...
							{
								if (((L2PetInstance) summon).wolf() || ((L2PetInstance) summon).hatchling() || ((L2PetInstance) summon).strider())
								{
									continue;
								}
	...
	public void addDamageHate(L2Character attacker, int damage, int aggro)
	{
		...
		{
			if (getAI() != null && getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE && aggro > 0)
			{
				try
				{
					getAI()//
				...
				catch (Exception e)
				{
					...
					{
						LOG.error("Character:" + getAI().getActor().getName() + " Attacker:" + attacker.getName() + " aggro:" + aggro);
						e.printStackTrace();
					}
				}
			}
	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		int dropChanceMultiplier = 1;
		
		if (this instanceof L2RaidBossInstance || (isRaid() && !(this instanceof L2GrandBossInstance)))
		{
			dropChanceMultiplier *= Config.ITEMS_RAID;
		}
		else if (this instanceof L2GrandBossInstance)
		{
			dropChanceMultiplier *= Config.ITEMS_BOSS;
		}
		else if (this instanceof L2MinionInstance)
		{
			dropChanceMultiplier *= Config.ITEMS_MINION;
		}
		else
		{
			dropChanceMultiplier *= Config.RATE_DROP_ITEMS;
		}
		...
		if (Config.DEEPBLUE_DROP_RULES)
		{
			if (levelModifier > 0)
			{
				...
				{
					deepBlueDrop *= dropChanceMultiplier;
				}
			}
		}
		...
		else if (isSweep)
		{
			...
			else
			{
				...
				if (Config.L2LIMIT_CUSTOM)
				{
					if (lastAttacker.isIncreasmentRatesHigh())
					{
						dropChance *= 1.50;
					}
					else if (lastAttacker.isIncreasmentRatesLow())
					{
						dropChance *= 1.25;
					}
				}
		...
		if (itemCount > 0)
		{
			return new RewardItem(drop.getItemId(), itemCount, 0);
		}
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		L2DropData drop = categoryDrops.dropOne(dropChanceMultiplier);
		...
		else
		{
			...
			else
			{
				...
				if (Config.L2LIMIT_CUSTOM)
				{
					if (lastAttacker.isIncreasmentRatesHigh())
					{
						categoryDropChance *= 1.50;
					}
					else if (lastAttacker.isIncreasmentRatesLow())
					{
						categoryDropChance *= 1.25;
					}
				}
			}
	...
	private int calculateLevelModifierForDrop(L2Character lastAttacker)
	{
		if (Config.DEEPBLUE_DROP_RULES)
		{
			...
			{
				return ((highestLevel - (getLevel())) * 20); // Option before: ((highestLevel - (getLevel() + 8)) * 9);
			}
			
			if (highestLevel - 6 >= getLevel())
			{
				return ((highestLevel - (getLevel() + 4)) * 6);
			}
		}
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
	{
		...
		// No drop at all for last character of box
		if (Config.L2UNLIMITED_CUSTOM)
		{
			if (player._active_boxes == 3)
			{
				List<String> playerBoxes = player.active_boxes_characters;
				
				if (playerBoxes != null && playerBoxes.size() > 2)
				{
					if (player.getName().contains(playerBoxes.get(2)))
					{
						return;
					}
				}
		...
		int levelModifier = calculateLevelModifierForDrop(lastAttacker); // level modifier in %'s (will be subtracted from drop chance)
			...
			if (cat.isSweep())
			{
				if (getSpoilerId() != 0)
				{
					...
					if (Config.L2JMOD_CHAMPION_ENABLE && isChampion() && Config.L2JMOD_CHAMPION_REWARD_SPOIL && (Config.DEEPBLUE_DROP_RULES && lastAttacker.getLevel() <= getLevel() + 5) && Config.L2JMOD_CHAMPION_REWARD > 0 && Rnd.get(100) < Config.L2JMOD_CHAMPION_REWARD)
					{
						...
						int champ_qty = Rnd.get(1, Config.L2JMOD_CHAMPION_REWARD_QTY);
			...
			else
			{
				...
				if (item != null)
				{
					...
					if (this instanceof L2RaidBossInstance || this instanceof L2GrandBossInstance)
					{
						...
						sm.addString(item.getCount() + (item.getEnchantLevel() > 0 ? " a +" + item.getEnchantLevel() : "")); // fix for enchanted items
		...
		{
			int champqty = Rnd.get(1, Config.L2JMOD_CHAMPION_REWARD_QTY);
	...
	public L2ItemInstance DropItem(L2PcInstance mainDamageDealer, RewardItem item)
	{
		...
		for (int i = 0; i < item.getCount(); i++)
		{
			...
			{
				// For the boss events
				if (getNpcId() >= 65000 && getNpcId() <= 65010)
				{
					ditem.getDropProtection().setProtected(false);
				}
			}
			
			ditem.dropMe(this, mainDamageDealer, newX, newY, newZ);
	...
	private void levelSoulCrystals(L2Character attacker)
	{
		...
		for (L2PcInstance player : players)
		{
			...
			// If succeeds or it is a full party absorb, level up the crystal. !!! Grand bosses allways have 100%, other raid bosses or mobs by chance from config. !!!
			// 25286 - Anakim and 25283 - Lilith 100% by L2OFF
			if (this instanceof L2RaidBossInstance && (getNpcId() == 25286 || getNpcId() == 25283) //
				|| this instanceof L2GrandBossInstance && (absorbType == L2NpcTemplate.AbsorbCrystalType.FULL_PARTY && (doLevelup) || absorbType == L2NpcTemplate.AbsorbCrystalType.PARTY_ONE_RANDOM && (doLevelup)) //
				|| dice <= chanceLevelUp)
			{
				// Check if player isn't too far from this (killed monster)
				if (!Util.checkIfInRange(Config.ALT_PARTY_RANGE, this, player, true))
				{
					continue;
				}
				// Give staged crystal
	...
	public void setSeeded(int id, int seederLvl)
	{
		// OLD system
		/*
		 * for (int skillId : getTemplate().getSkills().keySet()) { switch (skillId) { case 4303: // Strong type x2 count *= 2; break; case 4304: // Strong type x3 count *= 3; break; case 4305: // Strong type x4 count *= 4; break; case 4306: // Strong type x5 count *= 5; break; case 4307: // Strong
		 * type x6 count *= 6; break; case 4308: // Strong type x7 count *= 7; break; case 4309: // Strong type x8 count *= 8; break; case 4310: // Strong type x9 count *= 9; break; } } final int diff = getLevel() - L2Manor.getInstance().getSeedLevel(_seedType) - 5; // High level mobs bonus if (diff
		 * > 0) { count += diff; }
		 */
		...
		double cropPrice = L2Manor.getInstance().getCropBasicPriceBySeed(_seedType);
		double adenaMinDropCount = getTemplate().getMinAdenaDropCount();
		
		double ssBlueMinDropCount = getTemplate().getMinSsBlueDropCount();
		double ssGreenMinDropCount = getTemplate().getMinSsGreenDropCount();
		double ssRedMinDropCount = getTemplate().getMinSsRedDropCount();
		
		double ssBluePrice = ItemTable.getInstance().getTemplate(6360).getReferencePrice();
		double ssGreenPrice = ItemTable.getInstance().getTemplate(6361).getReferencePrice();
		double ssRedPrice = ItemTable.getInstance().getTemplate(6362).getReferencePrice();
		
		double allSsAmount;
		
		if (adenaMinDropCount == 0)
		{
			allSsAmount = Math.abs((ssBlueMinDropCount * ssBluePrice) + (ssGreenMinDropCount * ssGreenPrice) + (ssRedMinDropCount * ssRedPrice));
			adenaMinDropCount = allSsAmount;
		}
		
		int actualCount = Math.toIntExact((long) (adenaMinDropCount / cropPrice));
		double chanceToGetAnother = Math.round((adenaMinDropCount / cropPrice - actualCount) * 100);
		...
		if (Rnd.get(100) < chanceToGetAnother)
		{
			count += 1;
		}
		
		// Announcements _a = Announcements.getInstance();
		// _a.sys("adenaMinDropCount: " + adenaMinDropCount);
		// _a.sys("cropPrice: " + cropPrice);
		// _a.sys("actualCount: " + actualCount);
		// _a.sys("chanceToGetAnother: " + chanceToGetAnother);
		// _a.sys("-------------------------------------");
