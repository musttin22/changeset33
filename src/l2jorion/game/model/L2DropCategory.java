...
public class L2DropCategory
{
	...
	public void addDropData(L2DropData drop, int multiplier)
	{
		if (!drop.isQuestDrop())
		{
			_drops.add(drop);
			...
			_categoryBalancedChance += Math.min((drop.getChance() * multiplier), L2DropData.MAX_CHANCE);
		}
	}
	 ...
	 */
	public synchronized L2DropData dropOne(int multiplier)
	{
		...
		for (final L2DropData drop : getAllDrops())
		{
			sum += Math.min((drop.getChance() * multiplier), L2DropData.MAX_CHANCE);
