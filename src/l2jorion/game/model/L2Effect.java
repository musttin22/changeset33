...
public abstract class L2Effect
{
	...
	public void setRestoreType(final int type)
	{
		_restoreType = type;
	}
	...
	{
		setFirstTime(newfirsttime, 0, true);
	}
	
	public void setFirstTime(int newfirsttime, int restoreType, boolean skillMastery)
	{
		if (_currentFuture != null)
		{
			...
			if (!skillMastery)
			{
				...
				if (restoreType > 1)
				{
					...
					{
						L2PcInstance player = (L2PcInstance) _effected;
						
						switch (restoreType)
						{
							case 2: // Buffs from buffer
								multiplier = (float) 1.5;
								break;
							case 3: // Premium account
								multiplier = Config.PREMIUM_BUFF_MULTIPLIER;
								if (player.getPremiumService() == 0) // Reset premium
								{
									multiplier = 1;
									setRestoreType(0);
								}
								break;
							case 4: // Buffs time item
								multiplier = 4;
								if (player.getHourBuffs() == 0) // Reset item
								{
									multiplier = 1;
									setRestoreType(0);
								}
						...
						newTime = Math.round(_template.period * multiplier);
					}
				}
