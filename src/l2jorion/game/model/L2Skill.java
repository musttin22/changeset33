...
public abstract class L2Skill
{
	...
	public static final int SKILL_SNIPE = 313;
	...
	protected L2Skill(StatsSet set)
	{
		...
		if (_operateType == SkillOpType.OP_CHANCE || _operateType == SkillOpType.OP_PASSIVE && _triggeredId != 0)
		{
	...
	public final int setHitTime(int time)
	{
		return _hitTime = time;
	}
		...
		switch (targetType)
		{
			case TARGET_ONE:
			{
				...
				switch (skillType)
				{
					...
					case MAGE_BANE:
					case WARRIOR_BANE:
						if (checkPartyClanAlly(activeChar, target))
						{
			...
			case TARGET_BEHIND_AURA:
			{
				...
				for (L2Character obj : activeChar.getKnownList().getKnownTypeInRadius(L2Character.class, _skillRadius))
				{
					...
					// Skip provoke skill
					if (getId() == 286 && obj instanceof L2GuardInstance)
					{
						continue;
					}
				...
				if (activeChar.getParty() != null)
				{
					...
					for (final L2PcInstance partyMember : partyList)
					{
						...
						if (!partyMember.isDead() && Util.checkIfInRange(getSkillRadius(), activeChar, partyMember, true))
						{
							...
							if (src != null)
							{
								if (((src._inEventCTF || src._inEventDM || src._inEventTvT || src._inEventVIP) && (!trg._inEventCTF && !trg._inEventDM && !trg._inEventTvT && !trg._inEventVIP))
									|| ((trg._inEventCTF || trg._inEventDM || trg._inEventTvT || trg._inEventVIP) && (!src._inEventCTF && !src._inEventDM && !src._inEventTvT && !src._inEventVIP)))
								{
									continue;
								}
							}
			...
			case TARGET_PARTY_MEMBER:
			{
				if (target != null && !target.isDead() && (target == activeChar //
					|| (activeChar.getParty() != null && target.getParty() != null && activeChar.getParty().getPartyLeaderOID() == target.getParty().getPartyLeaderOID()) //
					...
					if (clan != null)
					{
						for (final L2Object newTarget : activeChar.getKnownList().getKnownObjects().values())
						{
							...
							if (src != null)
							{
								...
								if (((src._inEventCTF || src._inEventDM || src._inEventTvT || src._inEventVIP) && (!playerTarget._inEventCTF && !playerTarget._inEventDM && !playerTarget._inEventTvT && !playerTarget._inEventVIP))
									|| ((playerTarget._inEventCTF || playerTarget._inEventDM || playerTarget._inEventTvT || playerTarget._inEventVIP) && (!src._inEventCTF && !src._inEventDM && !src._inEventTvT && !src._inEventVIP)))
								{
									continue;
								}
							}
			...
			case TARGET_CLAN:
			{
				...
				else if (activeChar instanceof L2NpcInstance)
				{
					...
					for (L2Object newTarget : activeChar.getKnownList().getKnownObjects().values())
					{
						...
						{
							if (newTarget.isDead() || !Util.checkIfInRange(radius, activeChar, newTarget, true))
							{
								continue;
							}
	...
	public final L2Effect[] getEffects(final L2Character effector, final L2Character effected, boolean sellBuffsSystem)
	{
		return getEffects(effector, effected, false, false, false, false, sellBuffsSystem);
	}
	
	public final L2Effect[] getEffects(final L2Character effector, final L2Character effected)
	{
		return getEffects(effector, effected, false, false, false, true, false);
	}
	...
	public final L2Effect[] getEffects(final L2Character effector, final L2Character effected, final boolean ss, final boolean sps, final boolean bss, boolean useSkillMastery, boolean sellBuffsSystem)
	{
		if (isPassive())
		{
		...
		for (final EffectTemplate et : _effectTemplates)
		{
			...
			if (success)
			{
				...
				if (e != null)
				{
					...
					if (Config.PREMIUM_BUFF_MULTIPLIER > 1 || Config.RON_CUSTOM || sellBuffsSystem)
					{
						if (effected instanceof L2PcInstance)
						{
							L2PcInstance player = (L2PcInstance) effected;
							...
							if (!sellBuffsSystem && player.getPremiumService() == 1 && Config.PREMIUM_BUFF_MULTIPLIER > 1)
							{
								e.setRestoreType(3);
								multiplier = Config.PREMIUM_BUFF_MULTIPLIER;
							}
							...
							if (!sellBuffsSystem && Config.SKILL_DURATION_LIST.containsKey(e.getSkill().getId()))
							{
								float newTime = (e.getPeriod() * multiplier);
								e.setPeriod(Math.round(newTime));
								e.rescheduleEffect();
							}
							else if (sellBuffsSystem)
							{
								float newTime = (e.getPeriod() * multiplier);
								e.setPeriod(Math.round(newTime));
								e.rescheduleEffect();
						...
						else if (effected instanceof L2Summon)
						{
							L2Summon pet = (L2Summon) effected;
							...
							if (!sellBuffsSystem && pet.getOwner().getPremiumService() == 1 && Config.PREMIUM_BUFF_MULTIPLIER > 1)
							{
								e.setRestoreType(3);
								multiplier = Config.PREMIUM_BUFF_MULTIPLIER;
							}
							...
							if (Config.RON_CUSTOM)
							{
								if (pet.getOwner().getHourBuffs() == 1)
								{
									e.setRestoreType(4);
									multiplier = 4;
								}
			...
			if (e != null)
			{
				...
				else
				{
					if (e.getEffectType() == EffectType.BUFF)
					{
						...
						sm.addSkillName(e.getSkill());
						env.player.sendPacket(sm);
					}
	...
	public boolean checkPartyClanAlly(final L2Character activeChar, final L2Object target)
	{
		if (activeChar instanceof L2PcInstance && target instanceof L2PcInstance)
		{
			...
			if (((activeCh._inEventCTF || activeCh._inEventDM || activeCh._inEventTvT || activeCh._inEventVIP) && (!targetChar._inEventCTF && !targetChar._inEventDM && !targetChar._inEventTvT && !targetChar._inEventVIP))
				|| ((targetChar._inEventCTF || targetChar._inEventDM || targetChar._inEventTvT || targetChar._inEventVIP) && (!activeCh._inEventCTF && !activeCh._inEventDM && !activeCh._inEventTvT && !activeCh._inEventVIP)))
			{
				return true;
			}
			
			if ((activeCh._inEventDM && targetChar._inEventDM) || (activeCh._inEventTvT && targetChar._inEventTvT) || (activeCh._inEventCTF && targetChar._inEventCTF) || (activeCh._inEventVIP && targetChar._inEventVIP))
			{
				return false;
			}
			...
			// Same ally
			if (activeCh.getAllyId() != 0 && targetChar.getAllyId() != 0 && activeCh.getAllyId() == targetChar.getAllyId())
			{
				return true;
			}
		}
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			...
			if (targetPlayer != null)
			{
				...
				{
					if (player.getSiegeState() > 0 && player.isInsideZone(ZoneId.ZONE_SIEGE) && player.getSiegeState() == targetPlayer.getSiegeState())
					{
						return false;
					}
				}
				...
				if (!sourceInArena && !(targetPlayer.isInsideZone(ZoneId.ZONE_PVP)) && !(targetPlayer.isInsideZone(ZoneId.ZONE_SIEGE)))
				{
					if (skill.isOffensive())
					{
						if (target instanceof L2PcInstance)
						{
							if (((L2PcInstance) target).getKarma() == 0 && ((L2PcInstance) target).getPvpFlag() == 0)
							{
								return false;
							}
						}
						...
						{
							if (((L2Summon) target).getOwner().getKarma() == 0 && ((L2Summon) target).getOwner().getPvpFlag() == 0)
							{
								return false;
							}
						}
