...
import l2jorion.game.datatables.sql.OfflineTradeTable;
import l2jorion.game.datatables.sql.OfflineTradeTableWithBuffer;
...
public class TradeList
{
	...
	public void GetBuffs(L2PcInstance player, ItemRequest[] _items, int price)
	{
		...
		for (ItemRequest sk : _items)
		{
			...
			if (player.isSellingBuffForPet() && player.getPet() != null)
			{
				skill.getEffects(_owner, player.getPet());
			}
			else
			{
				
				skill.getEffects(_owner, player);
	...
	{
		// Gift of Seraphim
		L2Skill skill1 = SkillTable.getInstance().getInfo(4703, 7);
		// Blessing of Seraphim
		L2Skill skill2 = SkillTable.getInstance().getInfo(4702, 7);
		// Gift of Seraphim
		L2Skill skill12 = SkillTable.getInstance().getInfo(4703, 13);
		// Blessing of Seraphim
		L2Skill skill22 = SkillTable.getInstance().getInfo(4702, 13);
		
		// Gift of Queen
		L2Skill skill3 = SkillTable.getInstance().getInfo(4700, 7);
		// Blessing of Queen
		L2Skill skill4 = SkillTable.getInstance().getInfo(4699, 7);
		
		// Gift of Queen
		L2Skill skill32 = SkillTable.getInstance().getInfo(4700, 13);
		// Blessing of Queen
		L2Skill skill42 = SkillTable.getInstance().getInfo(4699, 13);
		...
		if (!_owner.BuffsList.contains(skill1) && !_owner.BuffsList.contains(skill2) && _owner.getClassId().getId() == 28)
		{
			_owner.BuffsList.add(skill1);
			_owner.BuffsList.add(skill2);
		}
		
		if (!_owner.BuffsList.contains(skill12) && !_owner.BuffsList.contains(skill22) && _owner.getClassId().getId() == 104)
		{
			_owner.BuffsList.add(skill12);
			_owner.BuffsList.add(skill22);
		}
		
		if (!_owner.BuffsList.contains(skill3) && !_owner.BuffsList.contains(skill4) && _owner.getClassId().getId() == 14)
		{
			_owner.BuffsList.add(skill3);
			_owner.BuffsList.add(skill4);
		}
		
		if (!_owner.BuffsList.contains(skill32) && !_owner.BuffsList.contains(skill42) && _owner.getClassId().getId() == 96)
		{
			_owner.BuffsList.add(skill32);
			_owner.BuffsList.add(skill42);
		}
	...
	public synchronized boolean PrivateStoreBuy(L2PcInstance player, ItemRequest[] items, int price)
	{
		...
		if (_owner.isInOfflineMode())
		{
			if (!Config.RON_CUSTOM)
			{
				OfflineTradeTable.getInstance().storeOffliner(_owner);
			}
			else
			{
				OfflineTradeTableWithBuffer.getInstance().storeOffliner(_owner);
			}
		}
		return true;
	}
