...
public class L2CubicInstance
{
	...
	public void getCubicTarget()
	{
		try
		{
			...
			if (ownerTarget instanceof L2Character && ownerTarget != _owner.getPet() && ownerTarget != _owner)
			{
				...
				if ((_owner.getPvpFlag() > 0 && !_owner.isInsideZone(ZoneId.ZONE_PEACE)) || _owner.isInsideZone(ZoneId.ZONE_PVP) || _owner._inEventTvT && TvT.is_started() || _owner._inEventCTF && CTF.is_started() || _owner._inEventDM && DM.is_started())
				{
					...
					if (enemy != null)
					{
						...
						if (enemy.getPvpFlag() == 0 && !enemy.isInsideZone(ZoneId.ZONE_PVP) && !(enemy._inEventTvT || enemy._inEventCTF || enemy._inEventDM))
						{
							targetIt = false;
						}
						
						if (enemy.isInsideZone(ZoneId.ZONE_PEACE) && !(enemy._inEventTvT || enemy._inEventCTF || enemy._inEventDM))
						{
							targetIt = false;
						}
	...
	public void useCubicContinuous(final L2CubicInstance activeCubic, final L2Skill skill, final L2Object[] targets)
	{
		for (final L2Character target : (L2Character[]) targets)
		{
			...
			{
				final boolean acted = Formulas.getInstance().calcCubicSkillSuccess(activeCubic, target, skill);
				if (!acted)
				{
		...
		public void run()
		{
			...
			try
			{
				...
				if (skill != null)
				{
					...
					{
						// Get % of current hp
						double percentageOfHp = (target.getCurrentHp() / target.getMaxHp() * 100f);
						// Default option for HP below 30%:
						double chance = 53;
						if (percentageOfHp > 60)
						{
							chance = 13;
						}
						else if (percentageOfHp > 30)
						{
							chance = 33;
						}
						
						// if (target.getMaxHp() - target.getCurrentHp() > skill.getPower()) // Old
						if (Rnd.get(100) < Math.round(chance)) // New like L2OFF
						{
							final L2Character[] targets =
							{
								target
