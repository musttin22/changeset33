...
public final class L2GuardInstance extends L2Attackable
{
	...
	public L2GuardInstance(final int objectId, final L2NpcTemplate template)
	{
		...
		ThreadPoolManager.getInstance().scheduleAiAtFixedRate(new ReturnTask(), RETURN_INTERVAL, Math.abs(RETURN_INTERVAL + Rnd.nextInt(60000)));
	}
	...
	{
		if (!isInsideRadius(getSpawn().getLocx(), getSpawn().getLocy(), 120, true))
		{
			clearAggroList();
