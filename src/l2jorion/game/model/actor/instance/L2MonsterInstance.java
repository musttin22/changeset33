...
public class L2MonsterInstance extends L2Attackable
{
		...
		ThreadPoolManager.getInstance().scheduleAi(new Runnable()
		{
			...
			public void run()
			{
				try
				{
					...
					if (!isRaid() && !isInCombat() && !isAlikeDead() && !isDead() && (mobSpawn != null) && !isInsideRadius(mobSpawn.getLocx(), mobSpawn.getLocy(), Config.MAX_DRIFT_RANGE * 2, false))
					{
	...
	{
		if (_minionList.hasMinions() && !_minionList.getSpawnedMinions().isEmpty())
		{
			for (final L2MinionInstance minion : _minionList.getSpawnedMinions())
			{
