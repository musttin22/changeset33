...
{
	public static final int INTERACTION_DISTANCE = 160;
	...
	public boolean isEventMob = false, _isEventMobHighRatesEvent = false, _isEventMobMiniRbEvent = false, _isSoloZoneMob = false, _isEventMobTvT = false, _isEventVIPNPC = false, _isEventVIPNPCEnd = false, _isEventMobDM = false, _isEventMobCTF = false, _isCTF_throneSpawn = false, _isCTF_Flag = false,
		_isEventMobTM = false;
	...
	private int _npctValue = 0;
	
	private L2Character _factionEnemy;
	private L2Character _factionHelp;
	...
	public boolean hasLongerHelpRange()
	{
		return _staticAIData.getLongerHelpRange() != 0;
	}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					else if (_isEventMobHighRatesEvent)
					{
						HighRatesEvent.showEventHtml(player, String.valueOf(getObjectId()));
					}
					else if (_isEventMobMiniRbEvent)
					{
						MiniRbEvent.showEventHtml(player, String.valueOf(getObjectId()));
					}
					else if (_isSoloZoneMob)
					{
						SoloZone.showEventHtml(player, String.valueOf(getObjectId()));
					}
	...
	public void onActionShift(L2GameClient client)
	{
		...
		if (player.getAccessLevel().isGm() || (Config.ALT_GAME_VIEWNPC))
		{
			...
			html.replace("%hpbar%", Util.generateBar(150, 12, (long) getCurrentHp(), getMaxHp(), "HP"));
			html.replace("%mpbar%", Util.generateBar(150, 12, (long) getCurrentMp(), getMaxMp(), "MP"));
			html.replace("%title%", String.valueOf(getTitle()));
			
			html.replace("%objid%", String.valueOf(getObjectId()));
			html.replace("%type%", String.valueOf(getTemplate().getType()).substring(2));
			html.replace("%race%", getTemplate().getRace().toString().substring(0, 1).toUpperCase() + getTemplate().getRace().toString().toLowerCase().substring(1));
			html.replace("%id%", String.valueOf(getTemplate().getNpcId()));
			html.replace("%lvl%", String.valueOf(getTemplate().getLevel()));
			html.replace("%name%", String.valueOf((getTemplate().getName().length() >= 25 ? getTemplate().getName().substring(0, 25) : getTemplate().getName())));
			...
			html.replace("%fId%", String.valueOf(getTemplate().getFactionId().isEmpty() ? "-" : getTemplate().getFactionId()));
	...
	public void onBypassFeedback(final L2PcInstance player, final String command)
	{
		...
		else if (command.startsWith("SupportMagicCustomFighter"))
		{
			makeSupportMagicCustom(player, false);
		}
		else if (command.startsWith("SupportMagicCustomMage"))
		{
			makeSupportMagicCustom(player, true);
		}
		...
		{
			if (getNpcId() == 31680 || getNpcId() == 31676)
			{
		...
		else if (command.startsWith("ReduceDeathPenalty"))
		{
			if (!player.reduceAdena("Penalty", 3600, this, true))
			{
				return;
			}
	...
	public String getHtmlPath(L2PcInstance player, int npcId, int val)
	{
		...
		if (npcId == PowerPackConfig.BUFFER_NPC)
		{
			if (val == 0)
			{
				synchronized (Buffer._visitedPages)
				{
					Buffer._visitedPages.put(player.getObjectId(), String.valueOf(0));
				}
	...
	public void showQuestWindowSingle(L2PcInstance player, String questId)
	{
		...
		else
		{
			...
			if (qs == null)
			{
				...
				{
					final Quest[] questList = player.getAllActiveQuests(); // player.getAllActiveQuestsForNpc();
	...
	public void makeSupportMagic(final L2PcInstance player)
	{
		...
		if (!Config.RON_CUSTOM && player.isCursedWeaponEquiped())
		{
			return;
		}
	...
	public void makeSupportMagicCustom(final L2PcInstance player, boolean mage)
	{
		if (player == null)
		{
			return;
		...
		if (player.isMageClass())
		{
			...
			{
				skill = SkillTable.getInstance().getInfo(skillId, PowerPackConfig.MAGE_SKILL_LIST.get(skillId));
				if (skill != null)
				{
		...
		else
		{
			...
			{
				skill = SkillTable.getInstance().getInfo(skillId, PowerPackConfig.FIGHTER_SKILL_LIST.get(skillId));
				if (skill != null)
				{
					skills_to_buff.add(skill);
				}
		...
		{
			sk.getEffects(player, player, false, false, false);
		}
	}
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		...
		/** Some Seven Signs implementation **/
		...
		if (Config.RON_CUSTOM)
		{
			if (npcId == 11)
			{
				AcademyManager.showMainWindow(player);
				return;
			}
		}
		...
		{
			Gatekeeper.getInstance().showChatData(player, null, String.valueOf(val));
			return;
		}
		
		if (npcId == Config.REBIRTH_NPC_ID)
		{
			RebirthSystem.getInstance().showChatWindow(player);
			return;
		}
	...
	public int getNpctValue()
	{
		return _npctValue;
	}
	
	public void setNpcValue(int val)
	{
		_npctValue = val;
	}
	
	public boolean isNpcValue(int val)
	{
		return _npctValue == val;
	}
	
	public final L2Character getFactionEnemy()
	{
		return _factionEnemy;
	}
	
	public final void setFactionEnemy(L2Character value)
	{
		_factionEnemy = value;
	}
	
	public final void setFactionHelp(L2Character value)
	{
		_factionHelp = value;
	}
	
	public final L2Character getFactionHelp()
	{
		return _factionHelp;
	}
