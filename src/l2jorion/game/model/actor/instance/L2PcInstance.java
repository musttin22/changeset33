...
public class L2PcInstance extends L2PlayableInstance
{
	...
	private boolean _hideCharacterEffects = false;
	...
	private boolean _cwPopUpMenuOn = true;
	protected boolean _bossObserve = false;
	private boolean cwHideMenu = false;
	
	private boolean characterHideHeroGlow = false;
	private boolean characterHideWeaponGlow = false;
	
	private boolean characterHideSkins = false;
	
	private boolean _increaseRatesLow = false;
	private boolean _increaseRatesHigh = false;
	private boolean _showKillingAnnouncements = true;
	
	private boolean _clanFreight = false;
	private boolean skillBonusAdded = false;
	
	// Rebirth skill
	private boolean _switchingRebirthSkill = false;
	...
	private boolean _votedForEvent = false;
	private boolean _voteMenu = false;
	...
	private int _autofarm_engine_radius = 200;
	...
	private HashMap<Integer, String> _armorSkins = new HashMap<>();
	private HashMap<Integer, String> _weaponSkins = new HashMap<>();
	private HashMap<Integer, String> _hairSkins = new HashMap<>();
	private HashMap<Integer, String> _faceSkins = new HashMap<>();
	private HashMap<Integer, String> _shieldSkins = new HashMap<>();
	
	// L2Raptor settings
	private int _topBoardPvpKills = 0;
	private int _topBoardPkKills = 0;
	private int _clanWarPoints = 0;
	private int _clanRaidPoints = 0;
	private int _playerFishingPoints = 0;
	...
	private String _dressMeShowSkinOption;
	
	private static final String INSERT_OR_UPDATE_CHARACTER_DRESSME_DATA = "INSERT INTO characters_dressme_data (obj_Id, armor_skins, armor_skin_option, weapon_skins, weapon_skin_option, hair_skins, hair_skin_option, face_skins, face_skin_option, shield_skins, shield_skin_option) VALUES (?,?,?,?,?,?,?,?,?,?,?) "
	...
	private static final String UPDATE_CHARACTER_SKILL_LEVEL_ON_KEEP_SUBCLASS_SKILLS = "UPDATE character_skills SET skill_level=? WHERE skill_id=? AND char_obj_id=?";
	...
	private static final String ADD_SKILL_SAVE_OR_UPDATE = "INSERT INTO character_skills_save (char_obj_id,skill_id,skill_level,effect_count,effect_cur_time,reuse_delay,systime,restore_type,class_index,buff_index) VALUES (?,?,?,?,?,?,?,?,?,?) "
		+ "ON DUPLICATE KEY UPDATE skill_level=?,effect_count=?,effect_cur_time=?,reuse_delay=?,systime=?,restore_type=?,class_index=?,buff_index=?";
	...
	public boolean _inEventMiniRb = false;
	public boolean _inEventHighRates = false;
	
	private boolean _inSoloZone = false;
	...
	private boolean _sellingBuffForPet = false;
	private boolean _sellingBuff = false;
	...
	public RequestBypassToServer dialogRqt = null;
	public RequestBypassToServer dialogRqtBuyAll = null;
	...
	public Quest[] getAllActiveQuests()
	{
		...
		for (QuestState qs : _quests.values())
		{
			if (qs == null || qs.isCompleted() || !qs.isCompleted() && !qs.isStarted() || qs.getQuest().getQuestId() > 999 || qs.getQuest().getQuestId() < 1)
			{
				continue;
			}
	...
	public void setClassId(int Id)
	{
		...
		if (getLvlJoinedAcademy() != 0 && _clan != null && PlayerClass.values()[Id].getLevel() == ClassLevel.Third)
		{
			...
			{
				AcademyManager.deleteDataAndAddReward(this);
			}
		}
		if (isSubClassActive())
		{
	...
	public boolean reduceAncientAdena(String process, int count, L2Object reference, boolean sendMessage)
	{
		...
		if (ancientAdenaItem == null || count > getAncientAdena())
		{
			if (sendMessage)
			{
				sendPacket(new SystemMessage(SystemMessageId.YOU_NOT_ENOUGH_ADENA));
	...
	public void onAction(L2PcInstance player)
	{
		...
		{
			if (getPrivateStoreType() != 0 || isSellingBuff())
			{
	...
	public void onActionShift(L2PcInstance player)
	{
		...
		if (player.isGM())
		{
			...
			else
			{
				if (isBot())
				{
					AdminEditChar.gatherCharacterInfo(player, this, "botinfo.htm");
				}
				else
				{
	...
	public void setTarget(L2Object newTarget)
	{
		...
		if (oldTarget != null)
		{
			...
			{
				if (!getAutoFarmMode().toLowerCase().contains("none") && oldTarget.isMonster() && ((L2NpcInstance) oldTarget).getSpoilerId() != 0)
				{
					// Improvement for autofarm mod
					return;
				}
			}
		...
		if (Config.PLAYER_SPAWN_PROTECTION > 0 || Config.PLAYER_TELEPORT_PROTECTION > 0)
		{
			if (!isGM())
			{
				if (newTarget != null && newTarget instanceof L2PcInstance)
				{
					if (((L2PcInstance) newTarget).isSpawnProtected())
					{
						sendMessage("The Spawn Protection is active.");
						sendPacket(ActionFailed.STATIC_PACKET);
						return;
					}
					
					if (((L2PcInstance) newTarget).isTeleportProtected())
					{
						sendMessage("The Teleport Spawn Protection is active.");
						sendPacket(ActionFailed.STATIC_PACKET);
						return;
					}
				}
	...
	public void useEquippableItem(L2ItemInstance item, boolean abortAttack)
	{
		...
		// Remove the item if it's equipped
		if (isEquiped)
		{
			...
			{
				if (item.getItem() instanceof L2Weapon && getWeaponSkinOption() > 0 && DressMeData.getInstance().getWeaponSkinsPackage(getWeaponSkinOption()) != null)
				{
					setWeaponSkinOption(0);
				}
				
				if (item.getItem() instanceof L2Weapon && getShieldSkinOption() > 0 && DressMeData.getInstance().getShieldSkinsPackage(getShieldSkinOption()) != null)
				{
					setShieldSkinOption(0);
				}
			}
			...
			// Remove skill Snipe's effect
			if (item.isWeapon() && ((L2Weapon) item.getItem()).getItemType() == L2WeaponType.BOW)
			{
				if (getFirstEffect(L2Skill.SKILL_SNIPE) != null)
				{
					removeEffect(L2Skill.SKILL_SNIPE);
					setIsImobilised(false);
				}
			}
			
			// Remove augmentation bonus on equipment
			if (item.isAugmented())
			{
	...
	public void sellBuffsMenu(L2PcInstance buffer, int page)
	{
		...
		tb.append("<html>");
		tb.append("<head>");
		tb.append("<title>Class: " + CharTemplateTable.getClassNameById(buffer.getClassId().getId()) + "   Lv: " + buffer.getLevel() + "</title>");
		tb.append("</head>");
		tb.append("<body>");
		
		if (Config.SELLBUFF_MP_COSUME)
		{
			tb.append("" + Util.generateBar(280, 12, (long) buffer.getCurrentMp(), buffer.getMaxMp(), "MP") + "");
			tb.append("<center><img height=-6>" + (int) Math.round(buffer.getCurrentMp()) + "</center><br1>");
		}
		
		tb.append("<center>Buffs: <font color=00FF00>" + buffer.BuffsList.size() + "</font> <font color=777777>|</font>  Each price: <font color=LEVEL>" + Util.formatAdena(buffer.getBuffPrice()) + "</font> Adena</center><br1>");
		tb.append("<center><table width=80><tr><td>Select pet:</td><td>%popupmenu%</td></tr></table>");
		tb.append("<table><tr>");
		...
		for (int i = BuffsStart; i < BuffsEnd; i++)
		{
			...
			tb.append("<td><button action=\"bypass sell_buff " + BuffsListForPlayers[i].getId() + "\" width=31 height=31 back=\"Icon.skill" + iconId + "\" fore=\"Icon.skill" + iconId + "\"></td>");
			tb.append("<td><img height=6><button value=\"" + BuffsListForPlayers[i].getName() + " (Lv " + BuffsListForPlayers[i].getLevel() + ")\" action=\"bypass sell_buff " + BuffsListForPlayers[i].getId() + " " + BuffsListForPlayers[i].getLevel() + " " + page
		...
		tb.append("<img height=-5><table><tr>");
		for (int x = 0; x < MaxPages; x++)
		{
			...
			{
				tb.append("<td width=20><a action=\"bypass buffspage " + x + "\">[" + pagenr + "]</a>&nbsp;&nbsp;</td>");
			}
		}
		...
		if (isSellingBuffForPet())
		{
			n.replace("%popupmenu%", "<button value=\"\" action=\"bypass change_target_for_buffs\" width=15 height=15 back=\"L2UI.CheckBox\" fore=\"L2UI.CheckBox_checked\">");
		}
		else
		{
			n.replace("%popupmenu%", "<button value=\"\" action=\"bypass change_target_for_buffs\" width=15 height=15 back=\"L2UI.CheckBox_checked\" fore=\"L2UI.CheckBox\">");
		}
				...
				else if (_inEventDM && pk._inEventDM)
				{
					if (DM.is_teleport() || DM.is_started())
					{
						...
						if (Config.DM_ENABLE_KILL_REWARD)
						{
							...
							pk.sendMessage("You have earned " + Config.DM_KILL_REWARD_AMOUNT + " item(s) of " + reward.getName() + ".");
						}
						
						sendMessage("You will be revived and teleported to spot in " + Math.abs(Config.DM_REVIVE_DELAY / 1000) + " seconds!");
						ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
						{
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		if (isCursedWeaponEquiped())
		{
			if (Config.RON_CUSTOM)
			{
				if (target instanceof L2PcInstance && Config.ANNOUNCE_PK_KILL && isShowKillingAnnouncements())
				{
					SystemMessage sm = new SystemMessage(SystemMessageId.S1_ASSASSINATED_PLAYER_S2_IN_S3);
					sm.addString((isCursedWeaponEquiped() ? L2Item.getItemNameById(getCursedWeaponEquipedId()) + " owner " : "") + "" + getName() + (getClan() != null ? " (Clan: " + getClan().getName() + ")" : ""));
		...
		if (Config.ALLOW_RANDOM_PVP_ZONE && isInsideZone(ZoneId.ZONE_RANDOM))
		{
			if (Config.PVP_ZONE_REWARD_ID > 0 && !isBot())
			{
				PlaySound ps = new PlaySound(0, "ItemSound.quest_itemget", 1, getObjectId(), getX(), getY(), getZ());
				sendPacket(ps);
				addItem("AutoLoot", Config.PVP_ZONE_REWARD_ID, Rnd.get(Config.PVP_ZONE_REWARD_AMOUNT_MIN, Config.PVP_ZONE_REWARD_AMOUNT_MAX), null, true);
			}
		}
		...
		{
			if (target instanceof L2PcInstance && Config.ANNOUNCE_PVP_KILL && isShowKillingAnnouncements())
			{
			...
			if (targetPlayer.getClan() != null && getClan() != null)
			{
				if (getClan().isAtWarWith(targetPlayer.getClanId()))
				{
					if (targetPlayer.getClan().isAtWarWith(getClanId()))
					{
						...
						if (Config.RON_CUSTOM)
						{
							setClanWarPoints(getClanWarPoints() + 1);
						}
	...
	public void increasePvpKills()
	{
		...
		if (Config.RON_CUSTOM)
		{
			setTopBoardPvpKills(getTopBoardPvpKills() + 1);
		}
	...
	public void QuakeSystem()
	{
		...
		switch (quakeSystem)
		{
			case 3:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is Dominating! Killed 3 times in a row.", txt, pvpmusic);
				break;
			case 6:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on a Rampage! Killed 6 times in a row.", txt2, pvpmusic2);
				break;
			case 9:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on a Killing Spree! Killed 9 times in a row.", txt3, pvpmusic3);
				break;
			case 12:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on a Monster Kill! Killed 12 times in a row.", txt4, pvpmusic4);
				break;
			case 15:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is Unstoppable! Killed 15 times in a row.", txt5, pvpmusic5);
				break;
			case 18:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on Ultra Kill! Killed 18 times in a row.", txt6, pvpmusic6);
				break;
			case 21:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is Godlike! Killed 21 times in a row.", txt7, pvpmusic7);
				break;
			case 24:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is Wicked Sick! Killed 24 times in a row.", txt8, pvpmusic8);
				break;
			case 27:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on a Ludricrous Kill! Killed 27 times in a row.", txt9, pvpmusic9);
				break;
			case 30:
				...
				Announcements.getInstance().quakeSystemToAll(getName() + " is on Holy Shit! Killed 30 times in a row.", txt10, pvpmusic10);
				break;
		}
	}
	...
	public void increasePkKillsAndKarma(L2Character target, int targLVL)
	{
		...
		if (!(target instanceof L2Summon))
		{
			...
			if (Config.RON_CUSTOM)
			{
				setTopBoardPkKills(getTopBoardPkKills() + 1);
			}
	...
	private static L2PcInstance restore(int objectId)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				{
					player.setCharacterHideSkins(rset.getInt("trade") == 1 ? true : false);
				}
				else
				{
				...
				player.setHideEffects(rset.getInt("effects") == 1 ? true : false);
	...
	private void restoreDressMeData()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				String shieldskinIds = rset.getString("shield_skins");
				if (shieldskinIds != null && shieldskinIds.length() > 0)
				{
					for (String s : shieldskinIds.split(","))
					{
						if (s == null)
						{
							continue;
	...
	public synchronized void store(boolean force, boolean saveEffects)
	{
		if (!isTryingSkin())
		{
			storeDressMeData();
	...
	private synchronized void storeDressMeData()
	{
		...
		try
		{
			...
			{
				String s = "";
				for (Entry<Integer, String> i : _armorSkins.entrySet())
				{
					s += String.valueOf(i.getKey()) + ",";
				}
			...
			{
				String s = "";
				for (Entry<Integer, String> i : _weaponSkins.entrySet())
				{
					s += String.valueOf(i.getKey()) + ",";
				}
			...
			{
				String s = "";
				for (Entry<Integer, String> i : _hairSkins.entrySet())
				{
					s += String.valueOf(i.getKey()) + ",";
				}
			...
			{
				String s = "";
				for (Entry<Integer, String> i : _faceSkins.entrySet())
				{
					s += String.valueOf(i.getKey()) + ",";
				}
			...
			{
				String s = "";
				for (Entry<Integer, String> i : _shieldSkins.entrySet())
				{
					s += String.valueOf(i.getKey()) + ",";
				}
			...
			statement.setInt(14, getArmorSkinOption());
			...
			statement.setInt(16, getWeaponSkinOption());
			...
			statement.setInt(18, getHairSkinOption());
			...
			statement.setInt(20, getFaceSkinOption());
			...
			else
			{
				...
				statement.setString(21, s);
			}
			statement.setInt(22, getShieldSkinOption());
	...
	private synchronized void storeCharBase()
	{
		...
		try
		{
			...
			if (Config.L2EMI_CUSTOM)
			{
				statement.setInt(67, isCharacterHideSkins() ? 1 : 0);
			}
			else
			{
			...
			statement.setInt(70, isHidingEffects() ? 1 : 0);
	...
	private synchronized void storeEffect(boolean storeEffects)
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement(ADD_SKILL_SAVE_OR_UPDATE);
			...
			for (L2Effect effect : getAllEffects())
			{
				...
				{
					if (!effect.getInUse() && effect.getEffectType() != EffectType.BUFF) // We don't want to save herb, but we need to save hidden buff.
					{
						continue;
					}
					...
					statement.setInt(8, effect.getRestoreType());
					...
					if (!chargeSkill)
					{
						statement.setInt(11, skill.getLevel());
					}
					else
					{
						final EffectCharge e = (EffectCharge) getFirstEffect(L2Effect.EffectType.CHARGE);
						if (e != null)
						{
							statement.setInt(11, e.getLevel());
						}
					}
					
					statement.setInt(12, effect.getCount());
					statement.setInt(13, effect.getTime());
					
					if (hasSkillReuse(skill.getReuseHashCode()))
					{
						TimeStamp t = getSkillReuseTimeStamp(skill.getReuseHashCode());
						statement.setLong(14, t.hasNotPassed() ? t.getReuse() : 0);
						statement.setDouble(15, t.hasNotPassed() ? t.getStamp() : 0);
					}
					else
					{
					...
					statement.setInt(16, effect.getRestoreType());
					statement.setInt(17, getClassIndex());
					statement.setInt(18, ++buff_index);
					statement.execute();
				}
			}
			...
			for (int hash : getSkillReuseTimeStamps().keySet())
			{
				...
				if (t != null && t.hasNotPassed())
				{
					...
					// Update
					statement.setInt(11, t.getSkillLvl());
					statement.setInt(12, -1);
					statement.setInt(13, -1);
					statement.setLong(14, t.getReuse());
					statement.setDouble(15, t.getStamp());
	...
	private void storeSkill(L2Skill newSkill, L2Skill oldSkill, int newClassIndex)
	{
		...
		try
		{
			...
			if (oldSkill != null && newSkill != null)
			{
				...
				{
					statement = con.prepareStatement(UPDATE_CHARACTER_SKILL_LEVEL_ON_KEEP_SUBCLASS_SKILLS);
					statement.setInt(1, newSkill.getLevel());
					statement.setInt(2, oldSkill.getId());
					statement.setInt(3, getObjectId());
					statement.execute();
			...
			{
				LOG.error("could not store new skill. its NULL");
			}
		}
		catch (Exception e)
		{
		...
		try
		{
			if (!Config.KEEP_SUBCLASS_SKILLS)
			{
				...
				while (rset.next())
				{
					...
					if (id > 11000) // Let's leave a bit more space for custom skills
					{
			...
			while (rset.next())
			{
				...
				if (activateEffects)
				{
					for (L2Effect effect : skill.getEffects(this, this))
					{
						...
						effect.setFirstTime(effectCurTime, restoreType, false);
					}
				}
			}
			rset.close();
	...
	public void useMagic(L2Skill skill, boolean ctrlPressed, boolean shiftPressed)
	{
		...
		if (isInDuel())
		{
			...
			{
				sendMessage("You cannot do this while dueling.");
				sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		}
	...
	public boolean checkPvpSkill(L2Object target, L2Skill skill, boolean srcIsSummon)
	{
		...
		if (target instanceof L2PlayableInstance)
		{
			L2PcInstance eventTarget = null;
			if (target instanceof L2PcInstance)
			{
				eventTarget = (L2PcInstance) target;
			}
			else if (target instanceof L2Summon)
			{
				eventTarget = ((L2Summon) target).getOwner();
			}
			...
			if (skill.isOffensive() //
				&& (_inEventTvT && eventTarget._inEventTvT && TvT.is_started() && !_teamNameTvT.equals(eventTarget._teamNameTvT)) //
				|| (_inEventCTF && eventTarget._inEventCTF && CTF.is_started() && !_teamNameCTF.equals(eventTarget._teamNameCTF)) //
				|| (_inEventDM && eventTarget._inEventDM && DM.is_started()) //
				|| (_inEventVIP && eventTarget._inEventVIP && VIP._started))
			{
				return true;
			}
	...
	public FastSet<Integer> getAutoSoulShots()
	{
		...
		for (final int itemId : _activeSoulShots)
		{
			...
			if (item != null)
			{
				if (magic)
				{
					if (!summon)
					{
						if (itemId == 2509 || itemId == 2510 || itemId == 2511 || itemId == 2512 || itemId == 2513 || itemId == 2514 //
							|| itemId == 3947 || itemId == 3948 || itemId == 3949 || itemId == 3950 || itemId == 3951 || itemId == 3952 || itemId == 5790)
						{
				...
				if (physical)
				{
					if (!summon)
					{
						if (itemId == 1463 || itemId == 1464 || itemId == 1465 || itemId == 1466 || itemId == 1467 || itemId == 1835 || itemId == 5789 || //
							itemId == 6535 || itemId == 6536 || itemId == 6537 || itemId == 6538 || itemId == 6539 || itemId == 6540) // Fishing shots)
						{
							handler = ItemHandler.getInstance().getItemHandler(itemId);
							if (handler != null)
							{
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		restoreEffects(Config.ALT_RESTORE_EFFECTS_ON_SUBCLASS_CHANGE, true);
		...
		{
			RebirthSystem.getInstance().restoreRebirthSkills(this);
		}
		...
		{
			// Bonus for HP reset
			if (isSkillBonusAdded())
			{
				setSkillBonusAdded(false);
			}
			
			// Other bonuses
			L2Skill bonus1 = getSkill(9002);
			L2Skill bonus2 = getSkill(9003);
			L2Skill bonus3 = getSkill(9004);
			
			// Bonus 3
			if (bonus3 != null && bonus3.getLevel() == 5)
			{
				getAppearance().setTitleColor(Integer.decode("0x0000FF"));
				broadcastUserInfo();
			}
			// Bonus 2
			else if (bonus2 != null && bonus2.getLevel() == 5)
			{
				getAppearance().setTitleColor(Integer.decode("0x00FF00"));
				broadcastUserInfo();
			}
			// Bonus 1
			else if (bonus1 != null && bonus1.getLevel() == 5)
			{
				getAppearance().setTitleColor(Integer.decode("0x00CCCC"));
				broadcastUserInfo();
			}
			else
			{
	...
	public void onPlayerEnter()
	{
		...
		if (isCursedWeaponEquipped())
		{
			CursedWeaponsManager.getInstance().getCursedWeapon(getCursedWeaponEquipedId()).cursedOnLogin();
		}
	}
	...
	public static void teleToTarget(L2PcInstance targetChar, L2PcInstance summonerChar, L2Skill summonSkill)
	{
		...
		if (itemConsumeId != 0 && itemConsumeCount != 0)
		{
			...
			targetChar.getInventory().destroyItemByItemId("Consume", itemConsumeId, itemConsumeCount, targetChar, summonerChar);
	...
	public void calculateDeathPenaltyBuffLevel(L2Character killer)
	{
		...
		if ((Rnd.get(100) <= Config.DEATH_PENALTY_CHANCE) && !killer.isPlayer() && !(getCharmOfLuck() && (killer instanceof L2GrandBossInstance || killer instanceof L2RaidBossInstance)) && !(isInsideZone(ZoneId.ZONE_PVP) || isInsideZone(ZoneId.ZONE_SIEGE) || isInsideZone(ZoneId.ZONE_PEACE))) // ZONE_PEACE
		{
	...
	private boolean _customEnchantSkill = false;
	...
	public void setCustomEnchantSkill(boolean option)
	{
		_customEnchantSkill = option;
	}
	
	public boolean isCustomEnchantSkill()
	{
		return _customEnchantSkill;
	}
	...
	public void setPunishLevel(PunishLevel state, long delayInMilliseconds)
	{
		switch (state)
		{
			...
			case JAIL: // Jail Player
			{
				...
				else if (_inEventMiniRb)
				{
					MiniRbEvent.onDisconnect(this);
				}
				else if (_inSoloZone)
				{
					SoloZone.onDisconnect(this);
				}
				else if (_inEventVIP)
				{
	...
	public void setHideEffects(boolean effects)
	{
		_hideCharacterEffects = effects;
	}
	
	public boolean isHidingEffects()
	{
		return _hideCharacterEffects;
	}
	
	public void setCwHideMenu(boolean option)
	{
		cwHideMenu = option;
	}
	
	public boolean getCwHideMenu()
	{
		return cwHideMenu;
	}
	
	public void setCharacterHideHeroGlow(boolean option)
	{
		characterHideHeroGlow = option;
	}
	
	public boolean isCharacterHideHeroGlow()
	{
		return characterHideHeroGlow;
	}
	
	public void setCharacterHideWeaponGlow(boolean option)
	{
		characterHideWeaponGlow = option;
	}
	
	public boolean isCharacterHideWeaponGlow()
	{
		return characterHideWeaponGlow;
	}
	
	public void setCharacterHideSkins(boolean option)
	{
		characterHideSkins = option;
	}
	
	public boolean isCharacterHideSkins()
	{
		return characterHideSkins;
	}
	...
	public boolean isSellingBuff()
	{
		return _sellingBuff;
	}
	
	public void setSellingBuff(boolean j)
	{
		_sellingBuff = j;
	}
	
	public boolean isSellingBuffForPet()
	{
		return _sellingBuffForPet;
	}
	
	public void setSellingBuffForPet(boolean j)
	{
		_sellingBuffForPet = j;
	}
	...
	{
		return _armorSkins.containsKey(skin);
	}
	
	public boolean hasWeaponSkin(int skin)
	{
		return _weaponSkins.containsKey(skin);
	}
	
	public boolean hasHairSkin(int skin)
	{
		return _hairSkins.containsKey(skin);
	}
	
	public boolean hasFaceSkin(int skin)
	{
		return _faceSkins.containsKey(skin);
	}
	
	public boolean hasShieldSkin(int skin)
	{
		return _shieldSkins.containsKey(skin);
	}
	...
	public void buyShieldSkin(int id)
	{
		...
		{
			_shieldSkins.put(id, "shield");
		}
	}
	
	public void deleteArmorSkin(int id)
	{
		if (_armorSkins.containsKey(id))
		{
			_armorSkins.remove(id);
		}
	}
	
	public void deleteWeaponSkin(int id)
	{
		if (_weaponSkins.containsKey(id))
		{
			_weaponSkins.remove(id);
		}
	}
	
	public void deleteHairSkin(int id)
	{
		if (_hairSkins.containsKey(id))
		{
			_hairSkins.remove(id);
		}
	}
	
	public void deleteFaceSkin(int id)
	{
		if (_faceSkins.containsKey(id))
		{
			_faceSkins.remove(id);
		}
	}
	
	public void deleteShieldSkin(int id)
	{
		if (_shieldSkins.containsKey(id))
		{
			_shieldSkins.remove(id);
		}
	}
	...
	public String getIpAddress()
	{
		...
		if (getClient() != null)
		{
			ip = getClient().getConnection().getInetAddress().getHostAddress();
		}
	...
	public void setAutoFarmEngineRadius(int radius)
	{
		_autofarm_engine_radius = radius;
	}
	
	public int getAutoFarmEngineRadius()
	{
		return _autofarm_engine_radius;
	}
	...
	public void insertOrUpdateWeeklyBoardData()
	{
		...
		{
			LOG.error("Error could not store insertOrUpdateWeeklyBoardData: ");
			if (Config.ENABLE_ALL_EXCEPTIONS)
			{
				e.printStackTrace();
			}
	...
	private L2ItemInstance _targetItem = null;
	private L2ItemInstance _refinerItem = null;
	
	public final void setTargetWeapon(L2ItemInstance item)
	{
		_targetItem = item;
	}
	
	public final L2ItemInstance getTargetWeapon()
	{
		return _targetItem;
	}
	
	public final void setRefinerItem(L2ItemInstance item)
	{
		_refinerItem = item;
	}
	
	public final L2ItemInstance getRefinerItem()
	{
		return _refinerItem;
	}
	
	public void insertOrUpdateTopBoardData()
	{
		Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection(false);
			PreparedStatement statement = con.prepareStatement("INSERT INTO weekly_top_board (obj_Id, char_name, clan_name, pvpkills, pkkills, war_points, raid_points, fishing_points) values (?,?,?,?,?,?,?,?) "
				+ "ON DUPLICATE KEY UPDATE clan_name = ?, pvpkills=?, pkkills=?, war_points=?, raid_points=?, fishing_points=?");
			...
			statement.setString(9, getClan() != null ? getClan().getName() : "");
			statement.setInt(10, getTopBoardPvpKills());
			statement.setInt(11, getTopBoardPkKills());
			statement.setInt(12, getClanWarPoints());
			statement.setInt(13, getClanRaidPoints());
			statement.setInt(14, getPlayerFishingPoints());
		...
		{
			LOG.warn("Error could not store insertOrUpdateDataTopBoard: " + e.getMessage(), e);
			if (Config.ENABLE_ALL_EXCEPTIONS)
			{
				e.printStackTrace();
			}
	...
	public void loadWeeklyTopBoard()
	{
		Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection(false);
			PreparedStatement statement = con.prepareStatement("SELECT pvpkills, pkkills, war_points, raid_points, fishing_points FROM weekly_top_board WHERE obj_Id=?");
			statement.setInt(1, getObjectId());
			ResultSet rset = statement.executeQuery();
			
			while (rset.next())
			{
				setTopBoardPvpKills(rset.getInt("pvpkills"));
				setTopBoardPkKills(rset.getInt("pkkills"));
				setClanWarPoints(rset.getInt("war_points"));
				setClanRaidPoints(rset.getInt("raid_points"));
				setPlayerFishingPoints(rset.getInt("fishing_points"));
			}
	...
	public int getTopBoardPkKills()
	{
		return _topBoardPkKills;
	}
	
	public void setTopBoardPkKills(int pkKills)
	{
		_topBoardPkKills = pkKills;
	}
	
	public int getTopBoardPvpKills()
	{
		return _topBoardPvpKills;
	}
	
	public void setTopBoardPvpKills(int pvpKills)
	{
		_topBoardPvpKills = pvpKills;
	}
	
	public int getClanWarPoints()
	{
		return _clanWarPoints;
	}
	
	public void setClanWarPoints(int warPoints)
	{
		_clanWarPoints = warPoints;
	}
	
	public int getClanRaidPoints()
	{
		return _clanRaidPoints;
	}
	
	public void setClanRaidPoints(int raidPoints)
	{
		_clanRaidPoints = raidPoints;
	}
	
	public int getPlayerFishingPoints()
	{
		return _playerFishingPoints;
	}
	
	public void setPlayerFishingPoints(int points)
	{
		_playerFishingPoints = points;
	}
	
	public boolean isIncreasmentRatesLow()
	{
		return _increaseRatesLow;
	}
	
	public boolean isIncreasmentRatesHigh()
	{
		return _increaseRatesHigh;
	}
	
	public boolean isShowKillingAnnouncements()
	{
		return _showKillingAnnouncements;
	}
	
	public void setIncreasmentRatesLow(boolean option)
	{
		_increaseRatesLow = option;
	}
	
	public void setIncreasmentRatesHigh(boolean option)
	{
		_increaseRatesHigh = option;
	}
	
	public void setShowKillingAnnouncements(boolean option)
	{
		_showKillingAnnouncements = option;
	}
	
	// Clan Freight (custom)
	public boolean isClanFreight()
	{
		return _clanFreight;
	}
	
	public void setClanFreight(boolean option)
	{
		_clanFreight = option;
	}
	
	public boolean isSkillBonusAdded()
	{
		return skillBonusAdded;
	}
	
	public void setSkillBonusAdded(boolean option)
	{
		skillBonusAdded = option;
	}
	
	public boolean isSwitchingRebirthSkill()
	{
		return _switchingRebirthSkill;
	}
	
	public void setSwitchingRebirthSkill(boolean option)
	{
		_switchingRebirthSkill = option;
	}
	
	public boolean isInSoloZone()
	{
		return _inSoloZone;
	}
	
	public void setInSoloZone(boolean option)
	{
		_inSoloZone = option;
	}
	
	public long getBuySeedsTime()
	{
		return _buySeedsTime;
	}
	
	public void setDressMeShowSkinOption(String option)
	{
		_dressMeShowSkinOption = option;
	}
	
	public String getDressMeShowSkinOption()
	{
		return _dressMeShowSkinOption;
	}
	
	public void setBuySeedsTime(long time)
	{
		_buySeedsTime = time;
	}
