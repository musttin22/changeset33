...
public class L2PetInstance extends L2Summon
{
	...
	class FeedTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				{
					getOwner().sendPacket(new SystemMessage(SystemMessageId.YOUR_PET_IS_VERY_HUNGRY));
					if (Rnd.get(100) < 35)
					{
						// go back to pet control item, or simply said, unsummon it
						// setCurrentFed(0);
						stopFeed();
						getOwner().sendPacket(new SystemMessage(SystemMessageId.STARVING_GRUMPY_AND_FED_UP_YOUR_PET_HAS_LEFT));
						unSummon(getOwner());
						return;
					}
				}
				
				final int foodId = L2PetDataTable.getFoodItemId(getTemplate().getNpcId());
				if (foodId == 0)
				{
					return;
				}
	...
	public L2PetInstance(final int objectId, final L2NpcTemplate template, final L2PcInstance owner, final L2ItemInstance control)
	{
		...
		if (_data.getPetMaxLoad() == 0)
		{
			setMaxLoad(54510);
		}
		else
		{
			setMaxLoad(_data.getPetMaxLoad());
		}
	...
	public void store()
	{
		...
		if (itemInst != null && getName() != null && itemInst.getCustomType2() == 0)
		{
			itemInst.setCustomType2(1);
			itemInst.updateDatabase();
			final InventoryUpdate iu = new InventoryUpdate();
			iu.addModifiedItem(itemInst);
			getOwner().sendPacket(iu);
		}
	}
	...
	{
		final L2ItemInstance item = getActiveWeaponInstance();
	...
	{
		final L2ItemInstance item = getInventory().getPaperdollItem(Inventory.PAPERDOLL_CHEST);
		return (item == null) ? 0 : item.getItemId();
	}
	...
	public boolean wolf()
	{
		return getNpcId() == 12077;
	}
	
	public boolean hatchling()
	{
		return (getNpcId() == 12311 || getNpcId() == 12312 || getNpcId() == 12313);
	}
	
	public boolean strider()
	{
		return (getNpcId() == 12526 || getNpcId() == 12527 || getNpcId() == 12528);
	}
