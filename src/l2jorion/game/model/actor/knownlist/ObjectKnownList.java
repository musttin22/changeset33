...
{
	protected static final Logger LOG = LoggerFactory.getLogger(ObjectKnownList.class);
	...
	public boolean addKnownObject(L2Object object)
	{
		...
		if (object.getInstanceId() != getActiveObject().getInstanceId())
		{
			return false;
		}
		...
		if (!object.isVisible() || !Util.checkIfInRange(getDistanceToWatchObject(object), getActiveObject(), object, true))
		{
			return false;
		}
	...
	public final void findObjects()
	{
		...
		if (getActiveObject() instanceof L2PlayableInstance)
		{
			for (L2WorldRegion regi : region.getSurroundingRegions()) // offer members of this and surrounding regions
			{
				...
				{
					if (_object != getActiveObject() && _object.isVisible())
					{
	...
	public void forgetObjects(boolean fullCheck)
	{
		...
		for (L2Object object : knownObjects)
		{
			...
			if (!fullCheck && !(object instanceof L2PlayableInstance) && !object.isDead())
			{
				continue;
			}
	...
	public final <A> Collection<A> getKnownType(Class<A> type)
	{
		...
		{
			if (obj == getActiveObject() || !type.isAssignableFrom(obj.getClass()))
			{
				continue;
			}
			
			if (obj.isBot() && getActiveObject().isBot())
			{
				continue;
			}
	...
	public final <A> Collection<A> getKnownTypeInRadius(Class<A> type, int radius)
	{
		...
		for (L2Object obj : getKnownObjects().values())
		{
			if (obj == getActiveObject() || !type.isAssignableFrom(obj.getClass()) || !Util.checkIfInRange(radius, getActiveObject(), obj, true))
			{
				continue;
			}
