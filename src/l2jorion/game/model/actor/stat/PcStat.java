...
public class PcStat extends PlayableStat
{
	...
	public boolean addExpAndSp(long addToExp, int addToSp)
	{
		...
		// Baby pets take away 10% exp of owner (Like L2OFF)
		if (activeChar.getPet() instanceof L2BabyPetInstance)
		{
			if (activeChar.getLevel() - activeChar.getPet().getLevel() <= Config.PARTY_XP_CUTOFF_LEVEL)
			{
				L2BabyPetInstance pet = (L2BabyPetInstance) activeChar.getPet();
				ratioTakenByPlayer = pet.getPetData().getOwnerExpTaken() / 100f;
		...
		{
			if (!activeChar.isSkillBonusAdded())
			{
				if (activeChar.getExp() > 6200000000L)
				{
					if (activeChar.getSkill(9001) == null)
					{
						activeChar.addSkill(SkillTable.getInstance().getInfo(9001, 1), true);
						activeChar.sendSkillList();
						activeChar.setSkillBonusAdded(true);
					}
					else
					{
						activeChar.setSkillBonusAdded(true);
