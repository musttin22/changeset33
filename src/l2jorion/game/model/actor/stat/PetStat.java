...
public class PetStat extends SummonStat
{
	...
	public final boolean addLevel(byte value)
	{
		...
		{
			setExp(L2PetDataTable.getInstance().getPetData(getActiveChar().getNpcId(), getLevel()).getPetMaxExp());
		}
		...
		// Quick temp fix for mp/hp heal
		getActiveChar().getStatus().setCurrentHp(getMaxHp());
		getActiveChar().getStatus().setCurrentMp(getMaxMp());
	...
	public void setLevel(final int value)
	{
		...
		getActiveChar().setPetData(L2PetDataTable.getInstance().getPetData(getActiveChar().getTemplate().getNpcId(), getLevel()));
	...
	public int getMaxHp()
	{
		int val = (int) calcStat(Stats.MAX_HP, getActiveChar().getPetData().getPetMaxHP(), null, null);
		
		if (val != _oldMaxHp)
		{
			_oldMaxHp = val;
			
			final L2PetInstance player = getActiveChar();
	...
	public int getMaxMp()
	{
		int val = (int) calcStat(Stats.MAX_MP, getActiveChar().getPetData().getPetMaxMP(), null, null);
