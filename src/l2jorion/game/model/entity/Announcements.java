...
public class Announcements
{
	...
	public List<List<Object>> getEventAnnouncements()
	{
		return _eventAnnouncements;
	}
	...
	public void quakeSystemToAll(String text, PacketServer textOnScreen, PacketServer music)
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers().values())
		{
			if (player != null && player.isOnline() == 1)
			{
				...
				if (textOnScreen != null && player.getScreentxt())
				{
					player.sendPacket(textOnScreen);
				}
				
				if (music != null)
				{
					player.sendPacket(music);
				}
			}
		}
	}
	...
	public void punishmentAnnounceToAll(String text)
	{
