...
public class CTF implements EventTask
{
				...
				synchronized (_players)
				{
					for (final L2PcInstance player : _players)
					{
						if (player != null)
						{
							...
							if (Config.CTF_ON_START_REMOVE_CUBICS)
							{
							...
							if (Config.CTF_ON_START_REMOVE_ALL_EFFECTS)
							{
								...
								if (Config.CTF_EVENT_BUFFS_SYSTEM)
								{
									ArrayList<L2Skill> AllBuffs = new ArrayList<>();
									
									if (player.isMageClass())
									{
										for (int skillId : Config.CTF_EVENT_MAGE_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.CTF_EVENT_MAGE_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
									...
									{
										for (int skillId : Config.CTF_EVENT_FIGHTER_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.CTF_EVENT_FIGHTER_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
	...
	private static boolean addPlayerOk(final String teamName, final L2PcInstance eventPlayer)
	{
		...
		if (Config.CTF_PROHIBITED_CLASSES_LIST.contains(eventPlayer.getClassId().getId()))
		{
		...
		synchronized (_players)
		{
			for (final L2PcInstance player : _players)
			{
				if (player != null && (player.isOnline() != 0) && (player._inEventCTF))
				{
					...
					else if (teamName == null && !Config.CTF_NO_TIE)
					{
						// TIE
						int minus_reward = 0;
						if (_topScore != 0)
