...
public class DM implements EventTask
{
	...
	public static Location _playerSpawnLoc;
	...
	public static boolean startTeleport()
	{
		...
		_joining = false;
		Announcements.getInstance().gameAnnounceToAll(_eventName + ": Teleport to your spot in 10 seconds!");
				...
				synchronized (_players)
				{
					for (final L2PcInstance player : _players)
					{
						if (player != null)
						{
							...
							if (Config.DM_ON_START_REMOVE_CUBICS)
							{
							...
							if (Config.DM_ON_START_REMOVE_ALL_EFFECTS)
							{
								...
								if (Config.DM_EVENT_BUFFS_SYSTEM)
								{
									ArrayList<L2Skill> AllBuffs = new ArrayList<>();
									
									if (player.isMageClass())
									{
										for (int skillId : Config.DM_EVENT_MAGE_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.DM_EVENT_MAGE_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
									...
									{
										for (int skillId : Config.DM_EVENT_FIGHTER_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.DM_EVENT_FIGHTER_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
							...
							else
							{
								...
								if (Config.DM_SPAWN_LOCATIONS_FROM_LIST)
								{
									_playerSpawnLoc = (Location) (DMSpawnLocations.getInstance().getAllSpawnLocations()).toArray()[Rnd.get(0, DMSpawnLocations.getInstance().getAllSpawnLocations().size() - 1)];
									_playerX = _playerSpawnLoc.getX();
									_playerY = _playerSpawnLoc.getY();
									_playerZ = _playerSpawnLoc.getZ();
								}
	...
	public static void finishEvent()
	{
		...
		else
		{
			...
			{
				Announcements.getInstance().gameAnnounceToAll(_eventName + ": TIE - No players win the match.");
				if (Config.DM_STATS_LOGGER)
				{
					LOG.info(_eventName + ": TIE - No players win the match.");
				}
			}
			else
			{
			...
			if (Config.RON_CUSTOM)
			{
				// L2EventZone zone = ZoneManager.getInstance().getZone(_maxlvl, _maxPlayers, _npcZ, L2EventZone.class);
			}
		}
	...
	private static boolean addPlayerOk(final L2PcInstance eventPlayer)
	{
		if (eventPlayer._active_boxes > 1 && !Config.ALLOW_DUALBOX_EVENT)
		{
			...
			if (players_in_boxes != null && players_in_boxes.size() > 1)
			{
				for (final String character_name : players_in_boxes)
				{
					...
					{
						eventPlayer.sendMessage("You already participated in event with another character.");
						return false;
					}
				}
			}
		...
		if (Config.DM_PROHIBITED_CLASSES_LIST.contains(eventPlayer.getClassId().getId()))
		{
			eventPlayer.sendMessage("You can't join with this class!");
			return false;
		}
	...
	public static void showEventHtml(final L2PcInstance eventPlayer, final String objectId)
	{
		try
		{
			...
			if (Config.RON_CUSTOM)
			{
				replyMSG.append("<br><center><table width=300><tr><td align=\"center\"><button value=\"BACK\" action=\"bypass -h custom_topboard get 10\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></td></tr></table></center>");
			}
	...
	private static void processTopPlayer()
	{
		if (Config.DM_NO_TIE)
		{
			int _largestTopKillsA = Integer.MIN_VALUE, _largestTopKillsB = Integer.MIN_VALUE;
			
			for (final L2PcInstance player : _players)
			{
				if (player._countDMkills > _largestTopKillsA)
				{
					_largestTopKillsB = _largestTopKillsA;
					_largestTopKillsA = player._countDMkills;
				}
				else if (player._countDMkills > _largestTopKillsB)
				{
					_largestTopKillsB = player._countDMkills;
				}
			}
			
			// TIE
			if (_largestTopKillsA == _largestTopKillsB)
			{
				_topKills = -1;
				return;
			}
	...
	{
		if (Config.DM_SPAWN_LOCATIONS_FROM_LIST && !DMSpawnLocations.getInstance().getAllSpawnLocations().isEmpty())
		{
			_playerSpawnLoc = (Location) (DMSpawnLocations.getInstance().getAllSpawnLocations()).toArray()[Rnd.get(0, DMSpawnLocations.getInstance().getAllSpawnLocations().size() - 1)];
			_playerX = _playerSpawnLoc.getX();
			_playerY = _playerSpawnLoc.getY();
