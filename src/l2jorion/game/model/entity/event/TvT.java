...
public class TvT implements EventTask
{
				...
				synchronized (_players)
				{
					for (final L2PcInstance player : _players)
					{
						if (player != null)
						{
							...
							if (Config.TVT_ON_START_REMOVE_CUBICS)
							{
							...
							if (Config.TVT_ON_START_REMOVE_ALL_EFFECTS)
							{
								...
								if (Config.TVT_EVENT_BUFFS_SYSTEM)
								{
									ArrayList<L2Skill> AllBuffs = new ArrayList<>();
									
									if (player.isMageClass())
									{
										for (int skillId : Config.TVT_EVENT_MAGE_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.TVT_EVENT_MAGE_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
									...
									{
										for (int skillId : Config.TVT_EVENT_FIGHTER_SKILL_LIST.keySet())
										{
											L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.TVT_EVENT_FIGHTER_SKILL_LIST.get(skillId));
											if (skill != null)
											{
												AllBuffs.add(skill);
											}
	...
	public static void finishEvent()
	{
		...
		if (_teamEvent)
		{
			...
			synchronized (_players)
			{
				...
				final L2PcInstance loser = findLoser(_players);
				...
				if (_topKills != 0)
				{
					...
					if (Config.TVT_ANNOUNCE_TEAM_STATS)
					{
						...
						if ((loser != null) && (!loser.equals(bestKiller)))
						{
							Announcements.getInstance().gameAnnounceToAll(_eventName + ": Top loser: " + loser.getName() + " - Dies: " + loser._countTvTdies);
						}
					}
					...
					rewardTeam(_topTeam, bestKiller, loser);
					...
					if (Config.TVT_STATS_LOGGER)
					{
						...
						if ((loser != null) && (!loser.equals(bestKiller)))
						{
							LOG.info("Top loser: " + loser.getName() + " - Dies: " + loser._countTvTdies);
						}
	...
	private static boolean addPlayerOk(final String teamName, final L2PcInstance eventPlayer)
	{
		...
		if (Config.TVT_PROHIBITED_CLASSES_LIST.contains(eventPlayer.getClassId().getId()))
		{
				...
				synchronized (_players)
				{
					...
					else
					{
						...
						else if (Config.TVT_EVEN_TEAMS.equals("SHUFFLE"))
						{
							...
							replyMSG.append("<center><font color=\"3366CC\">Teams will be randomly generated!</font></center><br>");
	 ...
	 * @param loser the looer
	 */
	public static void rewardTeam(final String teamName, final L2PcInstance bestKiller, final L2PcInstance loser)
	{
		synchronized (_players)
		{
			for (final L2PcInstance player : _players)
			{
				if (player != null && (player.isOnline() != 0) && (player._inEventTvT) && (!player.equals(loser)) && (player._countTvTkills > 0 || Config.TVT_PRICE_NO_KILLS))
				{
					...
					else if (teamName == null && !Config.TVT_NO_TIE)
					{
						// TIE
						int minus_reward = 0;
						if (_topKills != 0)
	 ...
	 * @return the l2 pc instance
	 */
	public static L2PcInstance findLoser(final Vector<L2PcInstance> players)
	{
		if (players == null)
		{
			return null;
		...
		for (final L2PcInstance player : players)
		{
			if ((loser == null) || (loser._countTvTdies < player._countTvTdies))
			{
				loser = player;
			}
		}
