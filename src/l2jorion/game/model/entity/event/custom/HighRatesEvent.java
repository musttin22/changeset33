...
public class HighRatesEvent implements EventTask
{
	protected static final Logger LOG = LoggerFactory.getLogger(HighRatesEvent.class);
	...
	protected static int _npcId = 0, _npcX = 0, _npcY = 0, _npcZ = 0, _playerX = 0, _playerY = 0, _playerZ = 0, _npcHeading = 0, _joinTime = 0, _eventTime = 0, _dayOfWeek = 0;
	...
	public static HighRatesEvent getNewInstance()
	{
		return new HighRatesEvent();
	}
	...
	public static boolean set_eventName(final String _eventName)
	{
		if (!is_inProgress())
		{
			HighRatesEvent._eventName = _eventName;
			return true;
		}
		return false;
	}
	...
	public static boolean set_eventDesc(final String _eventDesc)
	{
		if (!is_inProgress())
		{
			HighRatesEvent._eventDesc = _eventDesc;
			return true;
		}
		return false;
	}
	...
	public static boolean set_joiningLocationName(final String _joiningLocationName)
	{
		if (!is_inProgress())
		{
			HighRatesEvent._joiningLocationName = _joiningLocationName;
			return true;
		}
		return false;
	}
	...
	public static boolean set_joinTime(final int _joinTime)
	{
		if (!is_inProgress())
		{
			HighRatesEvent._joinTime = _joinTime;
			return true;
		}
		return false;
	}
	...
	public static boolean set_eventTime(final int _eventTime)
	{
		if (!is_inProgress())
		{
			HighRatesEvent._eventTime = _eventTime;
			return true;
		}
		return false;
	}
	...
	private static void spawnEventNpc()
	{
		...
		try
		{
			...
			_npcSpawn.getLastSpawn()._isEventMobHighRatesEvent = true;
			...
			if (_joining || _started || _teleport)
			{
				switch (seconds)
				{
					case 3600: // 1 hour left
						...
						{
							Announcements.getInstance().gameAnnounceToAll(_eventName + ": Joinable in " + _joiningLocationName + "");
							Announcements.getInstance().gameAnnounceToAll(_eventName + ": " + seconds / 60 / 60 + " hour till registration close");
						}
						else if (_started)
						{
	...
	{
		/*
		 * if (eventPlayer._active_boxes > 1 && !Config.ALLOW_DUALBOX_EVENT) { final List<String> players_in_boxes = eventPlayer.active_boxes_characters; if (players_in_boxes != null && players_in_boxes.size() > 1) { for (final String character_name : players_in_boxes) { final L2PcInstance player =
		 * L2World.getInstance().getPlayer(character_name); if (player != null && player._inEventHighRates) { eventPlayer.sendMessage("You already participated in event with another character."); return false; } } } }
		 */
		
		if (eventPlayer._inEventHighRates)
		{
	...
	public static void loadEventMobsData()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT * FROM high_rates_event_mobs");
	...
	public static void loadData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT * from high_rates_event");
			...
			while (rs.next())
			{
				...
				_dayOfWeek = rs.getInt("dayOfWeek");
			}
			DatabaseUtils.close(statement);
		}
		catch (final Exception e)
	...
	public static void saveData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("DELETE from mini_rb_event");
			statement.execute();
			DatabaseUtils.close(statement);
			
			statement = con.prepareStatement("INSERT INTO mini_rb_event (eventName, eventDesc, joiningLocation, npcId, npcX, npcY, npcZ, npcHeading, playerX, playerY, playerZ, joinTime, eventTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		try
		{
			...
			synchronized (_players)
			{
				...
				else if (!_started && _joining)
				{
					if (_players.contains(eventPlayer))
					{
						replyMSG.append("<center><font color=\"3366CC\">You participated already.</font><br>");
						
						replyMSG.append("<table width=200 border=\"0\"><tr>");
						replyMSG.append("<td><center><button value=\"Leave\" action=\"bypass -h npc_" + objectId + "_highratesevent_player_leave\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center></td>");
					...
					else
					{
						...
						replyMSG.append("<center><button value=\"Join\" action=\"bypass -h npc_" + objectId + "_highratesevent_player_join\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center><br>");
					}
				}
				else if (_started && !_joining)
				{
					replyMSG.append("<center>" + _eventName + " event is in progress.<br></center>");
					if (_players.contains(eventPlayer))
					{
						replyMSG.append("<center><button value=\"Enter\" action=\"bypass -h npc_" + objectId + "_highratesevent_player_enter\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center><br>");
					}
					else
					{
						replyMSG.append("<center><font color=\"3366CC\">You want to participate in the event?</font></center><br>");
	...
	public static void addPlayer(final L2PcInstance player)
	{
		...
		player._inEventHighRates = true;
		_savePlayers.add(player.getName());
		player.sendMessage(_eventName + ": You successfully registered to the event.");
	}
	
	public static void removePlayer(final L2PcInstance player)
	{
		if (player != null && player._inEventHighRates)
		{
			player._inEventHighRates = false;
			...
			player.sendMessage("Your participation in the event has been removed.");
			if (_started)
			{
				player.setInstanceId(0);
	...
	public static void cleanHighRatesEvent()
	{
		synchronized (_players)
		{
			for (final L2PcInstance player : _players)
	...
	public static void kickPlayerFromHighRatesEvent(final L2PcInstance playerToKick)
	{
		...
		synchronized (_players)
		{
			if (_joining)
			{
				...
				playerToKick._inEventHighRates = false;
			}
		}
