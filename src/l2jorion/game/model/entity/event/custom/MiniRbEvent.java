...
public class MiniRbEvent implements EventTask
{
	protected static final Logger LOG = LoggerFactory.getLogger(MiniRbEvent.class);
	...
	protected static int _npcId = 0, _npcX = 0, _npcY = 0, _npcZ = 0, _playerX = 0, _playerY = 0, _playerZ = 0, _npcHeading = 0, _joinTime = 0, _eventTime = 0;
	...
	public static MiniRbEvent getNewInstance()
	{
		return new MiniRbEvent();
	}
	...
	public static boolean set_eventName(final String _eventName)
	{
		if (!is_inProgress())
		{
			MiniRbEvent._eventName = _eventName;
			return true;
		}
		return false;
	}
	...
	public static boolean set_eventDesc(final String _eventDesc)
	{
		if (!is_inProgress())
		{
			MiniRbEvent._eventDesc = _eventDesc;
			return true;
		}
		return false;
	}
	...
	public static boolean set_joiningLocationName(final String _joiningLocationName)
	{
		if (!is_inProgress())
		{
			MiniRbEvent._joiningLocationName = _joiningLocationName;
			return true;
		}
		return false;
	}
	...
	public static boolean set_joinTime(final int _joinTime)
	{
		if (!is_inProgress())
		{
			MiniRbEvent._joinTime = _joinTime;
			return true;
		}
		return false;
	}
	...
	public static boolean set_eventTime(final int _eventTime)
	{
		if (!is_inProgress())
		{
			MiniRbEvent._eventTime = _eventTime;
			return true;
		}
		return false;
	}
	...
	private static void spawnEventNpc()
	{
		...
		try
		{
			...
			_npcSpawn.getLastSpawn()._isEventMobMiniRbEvent = true;
	...
	private static boolean addPlayerOk(final L2PcInstance eventPlayer)
	{
		 ...
		 * L2World.getInstance().getPlayer(character_name); if (player != null && player._inEventMiniRb) { eventPlayer.sendMessage("You already participated in event with another character."); return false; } } } }
		 */
		
		if (eventPlayer._inEventMiniRb)
		{
	...
	public static void loadEventMobsData()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT * FROM mini_rb_event_mobs");
	...
	public static void loadData()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT * from mini_rb_event");
			...
			synchronized (_players)
			{
				...
				else if (!_started && _joining)
				{
					if (_players.contains(eventPlayer))
					{
						...
						replyMSG.append("<td><center><button value=\"Leave\" action=\"bypass -h npc_" + objectId + "_minirbevent_player_leave\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center></td>");
					...
					else
					{
						...
						replyMSG.append("<center><button value=\"Join\" action=\"bypass -h npc_" + objectId + "_minirbevent_player_join\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center><br>");
					}
				}
				else if (_started && !_joining)
				{
					...
					if (_players.contains(eventPlayer))
					{
						replyMSG.append("<center><button value=\"Enter\" action=\"bypass -h npc_" + objectId + "_minirbevent_player_enter\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\"></center><br>");
					}
					else
					{
						replyMSG.append("<center><font color=\"3366CC\">You want to participate in the event?</font></center><br>");
	...
	{
		if (player != null && player._inEventMiniRb)
		{
			player._inEventMiniRb = false;
	...
	public static void cleanMiniRbEvent()
	{
		synchronized (_players)
		{
			for (final L2PcInstance player : _players)
	...
	public static void kickPlayerFromMiniRbEvent(final L2PcInstance playerToKick)
	{
		...
		synchronized (_players)
		{
			if (_joining)
			{
				...
				playerToKick._inEventMiniRb = false;
			}
		}
