...
public class Siege
{
	...
	private final List<L2FlameTowerInstance> _flameTowers = new FastList<>();
	...
	public void endSiege()
	{
		if (getIsInProgress())
		{
			...
			LOG.info(getClass().getSimpleName() + ": The siege of " + getCastle().getName() + " has finished! " + fmt.format(new Date(System.currentTimeMillis())));
			
			if (getCastle().getOwnerId() <= 0)
			{
				announceToPlayer("The siege of " + getCastle().getName() + " has ended in a draw.", false);
				
				LOG.info(getClass().getSimpleName() + ": The siege of " + getCastle().getName() + " has ended in a draw. " + fmt.format(new Date(System.currentTimeMillis())));
			}
	...
	public void midVictory()
	{
		if (getIsInProgress()) // Siege still in progress
		{
			...
			if (getCastle().getOwnerId() > 0)
			{
				...
				if (getDefenderClans().size() == 0) // If defender doesn't exist (Pc vs Npc) and only an alliance attacks
				{
	...
	public void killedCT(final L2NpcInstance ct)
	{
		...
		if (_controlTowerMaxCount > 0 && SiegeManager.getInstance().getControlTowerLosePenalty() > 0)
		{
			if (_controlTowerCount <= 0)
			{
				_defenderRespawnDelayPenalty = (_controlTowerMaxCount - _controlTowerCount) / (_controlTowerCount + 1) * SiegeManager.getInstance().getControlTowerLosePenalty();
			}
			else
			{
	...
	private void correctSiegeDateTime()
	{
		...
		if (getCastle().getSiegeDate().getTimeInMillis() + 7200000 < Calendar.getInstance().getTimeInMillis()) // 7200000 = 2h siege time
		{
