...
public class L2Multisell
{
	...
	public MultiSellListContainer getList(final int id)
	{
		...
		LOG.warn("L2Multisell: Player (" + (_player != null ? _player.getName() : "") + ") can't find list with id: " + id);
		return null;
	}
	 ...
	 * @return
	 */
	private MultiSellListContainer generateMultiSell(final int listId, final boolean inventoryOnly, final boolean allowEquipped, final L2PcInstance player, final double taxRate)
	{
		if (player != null)
		{
			_player = player;
		...
		if (inventoryOnly)
		{
			...
			{
				items = player.getInventory().getUniqueItemsByEnchantLevel(false, false, false, allowEquipped);
			}
			else
			{
				items = player.getInventory().getUniqueItems(false, false, false, allowEquipped);
			}
	...
	{
		SeparateAndSend(listId, player, inventoryOnly, true, taxRate);
	}
	
	public void SeparateAndSend(final int listId, final L2PcInstance player, final boolean inventoryOnly, final boolean allowEquipped, final double taxRate)
	{
		MultiSellListContainer list = generateMultiSell(listId, inventoryOnly, allowEquipped, player, taxRate);
	...
	private void updateReferencePrice(final MultiSellListContainer container)
	{
		for (final MultiSellEntry entry : container.getEntries())
		{
			...
			if (entry.getIngredients().size() == 1 && entry.getIngredients().get(0).getItemId() == 57)
			{
				...
				{
					LOG.warn(getClass().getSimpleName() + ": " + container.getListId() + " entryId:" + entry.getEntryId() + " has an ADENA price lower than buy price. Automatically updating it...");
