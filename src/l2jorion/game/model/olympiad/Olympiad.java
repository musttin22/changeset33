...
public class Olympiad
{
	...
	private void updateCompStatus()
	{
		...
		_scheduledCompStart = ThreadPoolManager.getInstance().scheduleAi(() ->
		{
			...
			{
				_gameAnnouncer = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(new OlympiadAnnouncer(), 5000, 500);
			}
	...
	public String getMillisToOlympiadEndInfo()
	{
		...
		String formattedElapsedTime = "The Olympiad ends in: " + days + " day" + (days > 1 ? "s" : "") + " " + hours + " hour" + (hours > 1 ? "s" : "") + " " + minutes + " minute" + (minutes > 1 ? "s" : "");
		
		if (time <= 0)
		{
			formattedElapsedTime = "The Olympiad ends in: waiting for the new status";
		}
