...
public class AutoSpawn
{
	...
	private void restoreSpawnData()
	{
		...
		{
			LOG.warn("AutoSpawnHandler: Could not restore spawn data: " + e.getMessage());
	...
	public boolean removeSpawn(final AutoSpawnInstance spawnInst)
	{
		synchronized (_Mobs)
		{
			...
			synchronized (_schedules)
			{
				...
				{
					LOG.error("AutoSpawnHandler: Could not auto spawn for NPC ID " + spawnInst._npcId + " (Object ID = " + spawnInst._objectId + "): " + e.getMessage());
					if (Config.ENABLE_ALL_EXCEPTIONS)
					{
						e.printStackTrace();
					}
	...
	private class AutoSpawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (respawnTime == 0L || time > respawnTime)
				{
					...
					if (mob != null)
					{
						...
						{
							Announcements.getInstance().announceToAll(mob.getName() + " has spawned near " + nearestTown + "!");
						}
					}
				}
				else
			...
			{
				LOG.error("AutoSpawnHandler: An error occurred while initializing spawn instance (ID: " + _objectId + "): " + e.getMessage());
				if (Config.ENABLE_ALL_EXCEPTIONS)
				{
					e.printStackTrace();
				}
		...
		public void run()
		{
			try
			{
				...
				for (final L2NpcInstance npcInst : npc_instances)
				{
					...
					if (Config.DEBUG)
					{
						LOG.info("AutoSpawnHandler: Spawns removed for spawn instance (Npc ID: " + spawnInst.getNpcId() + " | Object ID: " + _objectId + ").");
					}
				}
			}
			catch (final Exception e)
			{
				LOG.error("AutoSpawnHandler: An error occurred while despawning spawn (Object ID: " + _objectId + "): " + e.getMessage());
				if (Config.ENABLE_ALL_EXCEPTIONS)
				{
					e.printStackTrace();
				}
