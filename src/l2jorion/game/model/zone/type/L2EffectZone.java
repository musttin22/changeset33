...
public class L2EffectZone extends L2ZoneType
{
	...
	protected void onEnter(final L2Character character)
	{
		if (character.isPlayable())
		{
			...
			if (character.isPlayer() && _isShowDangerIcon)
			{
	...
	protected void onExit(final L2Character character)
	{
		if (character.isPlayable())
		{
			...
			for (final Entry<Integer, Integer> e : _skills.entrySet())
			{
				...
				character.removeEffect(skill.getId());
			}
	...
	private final class ApplySkill implements Runnable
	{
		...
		public void run()
		{
			if (isEnabled())
			{
				for (L2Character player : getCharactersInside())
				{
					if (player != null && !player.isDead())
					{
						...
						if (Rnd.get(100) < getChance())
						{
							for (final Entry<Integer, Integer> e : _skills.entrySet())
							{
								...
								if (skill != null && skill.checkCondition(player, player, false))
								{
									if (!player.isAffectedBySkill(e.getKey()))
									{
