...
import l2jorion.game.datatables.sql.OfflineBuffersData;
...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
	...
	public L2PcInstance loadCharFromDisk(int charslot)
	{
		...
		if (Config.RON_CUSTOM)
		{
			...
			if (character == null)
			{
				LOG.severe("Could not restore character in slot: " + charslot);
			}
		...
		if (character != null)
		{
			...
			character.sendPacket(new SystemMessage(SystemMessageId.ANOTHER_LOGIN_WITH_ACCOUNT));
	...
	public void close(PacketServer packet)
	{
		if (getConnection() != null)
		{
	...
	public String toString()
	{
		try
		{
			String address = getRealIpAddress();
			
			if (Config.RON_CUSTOM)
			{
				address = getConnection().getInetAddress().getHostAddress();
			}
			...
			try
			{
				...
				if (player != null)
				{
					if (!player.isKicked() && !OlympiadManager.getInstance().isRegistered(player) && !player.isInOlympiadMode() && !player.isInFunEvent() && ((player.isInStoreMode() && Config.OFFLINE_TRADE_ENABLE) || (player.isInCraftMode() && Config.OFFLINE_CRAFT_ENABLE)))
					{
						...
						if (!Config.RON_CUSTOM)
						{
							OfflineTradeTable.getInstance().storeOffliner(player);
						}
						else
						{
							OfflineTradeTableWithBuffer.getInstance().storeOffliner(player);
						}
						
						LogRecord record = new LogRecord(Level.INFO, "Entering offline mode (OFFLINE STORE)");
						record.setParameters(new Object[]
						{
							player.getObjectId(),
							player.getName(),
					...
					if (Config.SELLBUFF_SYSTEM_OFFLINE && player.isSellingBuff() && player.isSitting() && !player.isKicked() && !OlympiadManager.getInstance().isRegistered(player) && !player.isInOlympiadMode() && !player.isInFunEvent())
					{
						if (Config.L2EMI_CUSTOM && !player.isInsideZone(ZoneId.ZONE_PEACE))
						{
							cleanMe(fast);
							return;
						}
						...
						OfflineBuffersData.getInstance().storeOfflineBufferOnExit(player);
						
						LogRecord record = new LogRecord(Level.INFO, "Entering offline mode (SELL BUFFS)");
						record.setParameters(new Object[]
						{
							player.getObjectId(),
							player.getName(),
			...
			try
			{
				...
				if (player != null)
				{
					...
					else
					{
						...
						else if (player.isInSoloZone())
						{
							SoloZone.onDisconnect(player);
						}
					}
					...
					try
					{
						player.store();// player.store(_forcedToClose);
					}
					catch (Exception e2)
					{
						if (Config.ENABLE_ALL_EXCEPTIONS)
