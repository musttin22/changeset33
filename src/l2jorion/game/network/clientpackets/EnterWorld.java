...
public class EnterWorld extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		if (Config.ALLOW_DRESS_ME_SYSTEM && Config.TADUSE_CUSTOM)
		{
		...
		{
			RebirthSystem.getInstance().restoreRebirthSkills(activeChar);
		}
		...
		if (Config.TADUSE_CUSTOM)
		{
			...
			{
				if (HighRatesEvent._savePlayers.contains(activeChar.getName()))
				{
					HighRatesEvent.addDisconnectedPlayer(activeChar);
				}
			}
			
			if (EventManager.MINIRB_EVENT_ENABLED)
			{
				if (MiniRbEvent._savePlayers.contains(activeChar.getName()))
				{
					MiniRbEvent.addDisconnectedPlayer(activeChar);
				}
			}
		}
		...
		{
			// Add cubic
			L2Skill skill = SkillTable.getInstance().getInfo(4322, 1);
			L2Effect e = activeChar.getFirstEffect(skill);
			if (e != null)
			{
				activeChar.addCubic(3, 1, 1, 8, 30, 3600000, false);
			}
	...
	private void updateDatabase(L2PcInstance player, long premiumTime)
	{
		...
		try
		{
			...
			long premiumExpire = (System.currentTimeMillis() + premiumTime);
