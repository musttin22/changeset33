		...
		for (MultiSellIngredient e : entry.getIngredients())
		{
			if (e.getItemId() != 65336 && e.getItemId() != 65436)
			{
				for (MultiSellIngredient a : entry.getProducts())
				{
					...
					if (a.getItemId() == 57)
					{
						long adenaAmount = player.getInventory().getInventoryItemCount(57);
						long exchangeAmount = a.getItemCount();
						if (Math.abs(adenaAmount + (exchangeAmount * _amount)) > Integer.MAX_VALUE)
						{
							player.sendPacket(new SystemMessage(SystemMessageId.YOU_HAVE_EXCEEDED_QUANTITY_THAT_CAN_BE_INPUTTED));
							return;
						}
