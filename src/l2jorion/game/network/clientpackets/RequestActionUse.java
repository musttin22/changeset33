	...
	protected void runImpl()
	{
		...
		switch (_actionId)
		{
			...
			case 38: // pet mount
				...
				if (pet != null && pet.isMountable() && !activeChar.isMounted() && !activeChar.isBetrayed())
				{
					...
					else if (!pet.isDead() && !activeChar.isMounted())
					{
						...
						// Remove dances and songs
						activeChar.removeDancesAndSongs();
						
						final Ride mount = new Ride(activeChar.getObjectId(), Ride.ACTION_MOUNT, pet.getTemplate().getNpcId());
			...
			case 1040: // Swoop Cannon - Big Bang
				...
				break;
			// Customer's custom skills for pets
			case 1041:
				if (!(target instanceof L2DoorInstance))
				{
					useSkill(7065);
			...
			case 2000:
				...
				if ((TownManager.getInstance().getTown(9)) != null)
				{
					if (!activeChar.isInsideZone(ZoneId.ZONE_BUFFER) && !activeChar.isInsideZone(ZoneId.ZONE_CLANHALL))
					{
						activeChar.sendMessage("You can't sell buffs here. It's not buffs selling zone.");
						activeChar.sendPacket(new ExShowScreenMessage("You can't sell buffs here. It's not buffs selling zone.", 3000, 2, false));
				...
				{
					if (activeChar.isInsideZone(ZoneId.ZONE_NOSTORE))
					{
						activeChar.sendMessage("You can't sell buffs here. It's not buffs selling zone.");
						activeChar.sendPacket(new ExShowScreenMessage("You can't sell buffs here. It's not buffs selling zone.", 3000, 2, false));
						activeChar.sendPacket(new PlaySound("ItemSound3.sys_impossible"));
