	...
	protected void runImpl()
	{
		...
		if (_skillType == 0)
		{
			...
			for (final L2SkillLearn s : skills)
			{
				...
				if (sk == null || sk != skill || !sk.getCanLearn(player.getSkillLearningClassId()) || !player.hasTempAccess() && !sk.canTeachBy(npcid))
				{
					continue;
				}
				counts++;
			...
			if (Config.ANCIENT_CUSTOM && player.getPremiumService() == 0 && skill.getId() == L2Skill.SKILL_DIVINE_INSPIRATION)
			{
				player.sendMessage("This feature is available for the premium account only.");
				return;
			}
		...
		else
		{
			...
			{
				Shop.showSkillList(player, ClassId.values()[player.getClassId().getId()]);
			}
		}
