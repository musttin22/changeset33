...
public final class RequestBuyItem extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendMessage("Players with karma can't buy items.");
			return;
		}
		...
		{
			if (!player.isGM() && (target == null || !(target instanceof L2MerchantInstance || target instanceof L2FishermanInstance || target instanceof L2MercManagerInstance || target instanceof L2ClanHallManagerInstance || target instanceof L2CastleChamberlainInstance)
		...
		if (merchant != null)
		{
			...
			if (!player.isGM() && !player.hasTempAccess())
			{
				if (lists == null)
				{
