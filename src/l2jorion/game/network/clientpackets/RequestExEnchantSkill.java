...
public final class RequestExEnchantSkill extends PacketClient
{
	...
	protected void runImpl()
	{
		 ...
		 * Customer's system
		 **************************/
		if (Config.ALPHA_CUSTOM || Config.DESIMKE_CUSTOM || Config.TADUSE_CUSTOM)
		{
			if (player.isCustomEnchantSkill())
			{
				_requiredSp = 0;
				_requiredExp = 0;
				_rate = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_CHANCE;
				_itemCount = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_ITEM_COUNT;
				spbId = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_ITEM_ID;
			}
		}
		...
		if (player.getSp() >= _requiredSp)
		{
			...
			{
				if (Config.ES_SP_BOOK_NEEDED && (_skillLvl == 101 || _skillLvl == 141) || player.isCustomEnchantSkill()) // only first lvl requires book
				{
					if (spb == null || !player.destroyItem("Consume", spb.getObjectId(), _itemCount, null, true))// Haven't spellbook
					{
