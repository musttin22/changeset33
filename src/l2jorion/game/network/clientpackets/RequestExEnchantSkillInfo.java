...
public final class RequestExEnchantSkillInfo extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		{
			if (activeChar.isCustomEnchantSkill())
			{
				requiredSp = 0;
				requiredExp = 0;
				rate = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_CHANCE;
				spbId = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_ITEM_ID;
				itemCount = PowerPackConfig.GMSHOP_SKILL_ENCHNANT_ITEM_COUNT;
			}
		}
		
		ExEnchantSkillInfo asi = new ExEnchantSkillInfo(skill.getId(), skill.getLevel(), requiredSp, requiredExp, rate);
		if (Config.ES_SP_BOOK_NEEDED && (skill.getLevel() == 101 || skill.getLevel() == 141) || activeChar.isCustomEnchantSkill()) // only first lvl requires book
		{
			asi.addRequirement(4, spbId, itemCount, 0);
		}
