...
public final class RequestJoinParty extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		{
			requestor.sendMessage("You can not invite into party so fast.");
			return;
		}
		...
		if (requestor.isSellingBuff())
		{
			requestor.sendPacket(new SystemMessage(SystemMessageId.INCORRECT_TARGET));
			requestor.sendMessage("You can't invite any player into party.");
			return;
		}
		...
		{
			requestor.sendPacket(new SystemMessage(SystemMessageId.INCORRECT_TARGET));
			requestor.sendMessage("You can't invite this player into party.");
			return;
		}
		
		if ((requestor._inEventDM && (DM.is_teleport() || DM.is_started())) || (target._inEventDM && (DM.is_teleport() || DM.is_started())))
		{
			requestor.sendMessage("You can't invite that player into party.");
			return;
		}
		...
		{
			requestor.sendMessage("You can't invite this player into party: you or your target are in event.");
			return;
		}
		...
		if (target.isInOfflineMode())
		{
			...
			{
				String msg = requestor.getName() + " just invited offline store (" + target.getName() + ") into party.";
				GmListTable.broadcastMessageToGMs(msg);
				Log.add(msg, "PartyInvitesToOfflineStores");
			}
			return;
		}
