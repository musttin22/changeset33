...
public final class RequestRestart extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		// Check if player is moving/running - NEW VALIDATION
		if (player.isMoving())
		{
			if (Config.DEBUG)
			{
				LOG.warn("Player " + player.getName() + " tried to restart while moving/running.");
			}
			
			player.sendMessage("Cannot restart while moving. Please stop first.");
			sendPacket(RestartResponse.valueOf(false));
			return;
		}
