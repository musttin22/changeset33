...
public final class RequestRestartPoint extends PacketClient
{
	...
	class DeathTask implements Runnable
	{
		...
		public void run()
		{
			...
			{
				if (activeChar._inEventMiniRb && MiniRbEvent.is_started() || activeChar._inEventHighRates && HighRatesEvent.is_started())
				{
			...
			// Solo Zone
			SoloZone.onDisconnect(activeChar);
			...
			try
			{
				...
				{
					if (Dungeon.getNewInstance().getPlayers().contains(activeChar))
					{
						activeChar.setInstanceId(0);
						Dungeon.getNewInstance().getPlayers().remove(activeChar);
					}
					
					if (PartyZone.getNewInstance().getPlayers().contains(activeChar))
					{
						activeChar.setInstanceId(0);
						PartyZone.getNewInstance().getPlayers().remove(activeChar);
					}
				}
