...
public final class Say2 extends PacketClient
{
	...
	public final static int PARTYROOM_COMMANDER = 15; // (red)
	public final static int PARTYROOM_ALL = 16; // (yellow)
	...
	protected void runImpl()
	{
		...
		if (activeChar == null)
		{
			LOG.warning("Say2: Character is null.");
			return;
		}
		...
		// Custom message
		if ((Config.ANCIENT_CUSTOM || Config.L2LIMIT_CUSTOM) && activeChar.getPremiumService() > 0 && _type == SHOUT)
		{
			cs = new CreatureSay(activeChar.getObjectId(), HERO_VOICE, activeChar.getName() + " [PREMIUM]", _text);
		}
		...
		switch (_type)
		{
			...
			case TRADE:
				...
				break;
			case ALL:
				if (Config.PREMIUM_CHAT)
				{
					if (_text.startsWith(Config.PREMIUM_CHAT_SYMBOL) && activeChar.getPremiumService() >= 1)
					{
						String text = _text;
						String reformatedText = (text).substring(1);
						
						CreatureSay pcs = new CreatureSay(activeChar.getObjectId(), PARTYROOM_ALL, "" + (Config.SHOW_TIME_IN_CHAT ? "[" + fmt.format(new Date(System.currentTimeMillis())) + "]" : "") + " " + activeChar.getName(), reformatedText);
