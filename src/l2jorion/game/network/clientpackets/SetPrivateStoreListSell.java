...
public class SetPrivateStoreListSell extends PacketClient
{
	...
	protected void runImpl()
	{
		...
		else
		{
			...
			{
				if (!player.isInsideZone(ZoneId.ZONE_BUFFER) && !player.isInsideZone(ZoneId.ZONE_CLANHALL))
				{
					player.sendPacket(new PrivateStoreManageListSell(player));
					player.sendMessage("You can't sell buffs here. It's not buffs selling zone.");
					player.sendPacket(new ExShowScreenMessage("You can't sell buffs here. It's not buffs selling zone.", 3000, 2, false));
