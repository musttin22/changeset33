...
public class CharInfo extends PacketServer
{
	...
	public CharInfo(L2PcInstance player)
	{
		...
		if (!hideInfo())
		{
			...
			else if (player.isSellingBuff())
			{
				String className = CharTemplateTable.getClassNameById(player.getClassId().getId());
				_title = "SB | " + className + " " + player.getLevel();
			}
			else
			{
			...
			if ((Config.OFFLINE_CRAFT_ENABLE || Config.OFFLINE_TRADE_ENABLE) && _activeChar.isInOfflineMode() && !_activeChar.isSellingBuff() && Config.OFFLINE_SET_NAME_COLOR)
			{
				_nameColor = Config.OFFLINE_NAME_COLOR;
			}
			else
			{
	...
	protected final void writeImpl()
	{
		...
		else
		{
			...
			if (Config.ALLOW_DRESS_ME_SYSTEM && !_activeChar.isCharacterHideSkins())
			{
			...
			writeC((_activeChar.isHero() && !_activeChar.isCharacterHideHeroGlow() || (_activeChar.isGM() && Config.GM_HERO_AURA) || _activeChar.getIsPVPHero()) ? 1 : 0); // Hero Aura
