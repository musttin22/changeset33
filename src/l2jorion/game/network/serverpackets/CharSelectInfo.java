...
public class CharSelectInfo extends PacketServer
{
	...
	protected final void writeImpl()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (Config.ALLOW_DRESS_ME_SYSTEM)
			{
				...
				if (charInfoPackage.getShieldSkinOption() > 0 && getShieldOption(charInfoPackage.getShieldSkinOption()) != null)
				{
					writeD(getShieldOption(charInfoPackage.getShieldSkinOption()).getShieldId() != 0 ? getShieldOption(charInfoPackage.getShieldSkinOption()).getShieldId() : charInfoPackage.getPaperdollObjectId(Inventory.PAPERDOLL_LHAND));
				}
				else
				{
			...
			if (Config.ALLOW_DRESS_ME_SYSTEM)
			{
				...
				if (charInfoPackage.getShieldSkinOption() > 0 && getShieldOption(charInfoPackage.getShieldSkinOption()) != null)
				{
					writeD(getShieldOption(charInfoPackage.getShieldSkinOption()).getShieldId() != 0 ? getShieldOption(charInfoPackage.getShieldSkinOption()).getShieldId() : charInfoPackage.getPaperdollItemId(Inventory.PAPERDOLL_LHAND));
				}
				else
				{
	...
	private void loadCharacterDressMeInfo(final CharSelectInfoPackage charInfopackage, final int ObjectId)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			final PreparedStatement statement = con.prepareStatement("SELECT obj_Id, armor_skins, armor_skin_option, weapon_skins, weapon_skin_option, hair_skins, hair_skin_option, face_skins, face_skin_option, shield_skins, shield_skin_option FROM characters_dressme_data WHERE obj_id=?");
			...
			if (chardata.next())
			{
				...
				charInfopackage.setShieldSkinOption(chardata.getInt("shield_skin_option"));
			}
