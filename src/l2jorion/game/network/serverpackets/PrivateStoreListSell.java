...
public class PrivateStoreListSell extends PacketServer
{
	...
	public PrivateStoreListSell(L2PcInstance player, L2PcInstance storePlayer)
	{
		...
		{
			changeTargetHtml(player, _storePlayer);
	...
	public static void changeTargetHtml(L2PcInstance player, L2PcInstance buffer)
	{
		if (player.getPet() == null)
		{
			player.setSellingBuffForPet(false);
		}
		
		String n = "<html><title>Buffer: " + buffer.getName() + "   Lv: " + buffer.getLevel() + "</title>"//
			+ "<body><center>"//
			+ "<img height=100>Buffs for: <font color=3399ff>" + (player.isSellingBuffForPet() ? "Pet" : "Player") + "</font><br><button value=\"Change\" action=\"bypass change_buffs_target_custom\" width=134 height=21 back=\"L2UI_ch3.BigButton3_over\" fore=\"L2UI_ch3.BigButton3\"><br>" //
			+ "</body></html>";
		player.sendPacket(new NpcHtmlMessage(n));
	}
