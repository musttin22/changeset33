...
public class Buffer implements IVoicedCommandHandler, ICustomByPassHandler, ICommunityBoardHandler
{
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		else if (activeChar.isCastingNow() && !Config.RON_CUSTOM || activeChar.isCastingPotionNow())
		{
	...
	public void handleCommand(String command, final L2PcInstance player, String parameters)
	{
		...
		{
			// Obtener el ID de la clase actual del jugador
			int classId = player.getClassId().getId();
			
			// Bloquear acceso a buffs de pet si:
			// - Es Necromancer (10) o Soultaker (94)
			// - Está en subclase
			if (classId == 10 || classId == 94 || player.isSubClassActive())
			{
				player.sendMessage("Tu clase o subclase no puede usar buffs para pets.");
				player.sendPacket(new ExShowScreenMessage("Tu clase o subclase no puede usar buffs para pets.", 2000, 2, false));
				player.sendPacket(new PlaySound("ItemSound3.sys_impossible"));
				returnHtm(command, "Chat", player);
				return;
			}
			
			// Verificar si el jugador tiene una pet activa
			...
			// Si pasa todo, continúa con el buff al pet normalmente
		}
		...
		{
			if (player.getLevel() > PowerPackConfig.BUFFER_FREE_LVL && player.getInventory().getAdena() < PowerPackConfig.BUFFER_PRICE)
			{
			...
			{
				player.reduceAdena("Buff", PowerPackConfig.BUFFER_PRICE, null, true);
			}
			
			returnHtm(command, "Chat", player);
		}
		// XXX
		else if (parameters.startsWith("SupportMagicCustom"))
		{
			...
			{
				if (sk.getSkillType() == SkillType.SUMMON)
				{
					player.doCast(sk);
				}
				else
				{
					sk.getEffects(player, player, false, false, false, false, false);
				}
			}
			...
			if (player.getPet() != null)
			{
				for (L2Skill sk : skills_to_buff)
				{
					sk.getEffects(player, player.getPet(), false, false, false, false, false);
				}
			}
				...
				if (skill != null)
				{
					if (player.getLevel() >= buff._minLevel && player.getLevel() <= buff._maxLevel)
					{
						...
						if (!buff._force && buffer != null)
						{
							...
							skill.getEffects(buffer, player, false, false, false, false, false);
							buffer.setBusy(false);
						}
						else
						{
							skill.getEffects(player, player, false, false, false, false, false);
						}
					}
				}
			}
				...
				if (skill != null)
				{
					if (player.getLevel() >= buff._minLevel && player.getLevel() <= buff._maxLevel)
					{
						...
						if (!buff._force && buffer != null)
						{
							...
							skill.getEffects(buffer, player.getPet(), false, false, false, false, false);
							buffer.setBusy(false);
						}
						else
						{
							skill.getEffects(player, player.getPet(), false, false, false, false, false);
						}
					}
				}
			}
				...
				if (target != null)
				{
					for (Scheme sk : BuffsTable.getInstance().getScheme(player.getObjectId(), scheme_key))
					{
						...
						if (buffer != null)
						{
							...
							skill.getEffects(buffer, target, false, false, false, false, false);
							buffer.setBusy(false);
						}
						else
						{
							skill.getEffects(target, target, false, false, false, false, false);
						}
					}
				}
				else
		...
		else if (currentCommand.startsWith("fighterbuff") || currentCommand.startsWith("magebuff"))
		{
			...
			if (target != null)
			{
				for (L2Skill sk : skills_to_buff)
				{
					sk.getEffects(target, target, false, false, false, false, false);
				}
	...
	private void showManageSchemeWindow(String command, L2PcInstance player, String targetType)
	{
		...
		if (BuffsTable.getInstance().getAllSchemes(player.getObjectId()) != null)
		{
			for (Entry<String, ArrayList<Scheme>> e = BuffsTable.getInstance().getAllSchemes(player.getObjectId()).head(), end = BuffsTable.getInstance().getAllSchemes(player.getObjectId()).tail(); (e = e.getNext()) != end;)
			{
				...
				if (color == 1)
				{
					tb.append("<table width=300 border=0 bgcolor=000000>");
					color = 2;
				}
				else
				{
				...
				tb.append("</tr>");
				tb.append("<tr>");
				tb.append("<td width=\"140\">Buffs: <font color=LEVEL>" + String.valueOf(BuffsTable.getInstance().getScheme(player.getObjectId(), e.getKey()).size()) + "</font> Price: <font color=LEVEL>" + cost + "</font></td>");
