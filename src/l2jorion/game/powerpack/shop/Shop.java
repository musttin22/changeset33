...
public class Shop implements IVoicedCommandHandler, ICustomByPassHandler, ICommunityBoardHandler
{
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		{
			if (!activeChar.isInsideZone(ZoneId.ZONE_PEACE) && !activeChar.isInsideZone(ZoneId.ZONE_CASTLE) && activeChar.getPremiumService() != 5 && activeChar.getLevel() > 40)
			{
				msg = "You can't use AIO outside town.";
	...
	public void handleCommand(String command, L2PcInstance player, String parameters)
	{
		...
		else if (parameters.startsWith("spec_exchange_multisell"))
		{
			try
			{
				...
				player.setTempAccess(true);
				L2Multisell.getInstance().SeparateAndSend(listId, player, true, false, 0);
			}
			catch (Exception e)
			{
				if (Config.ENABLE_ALL_EXCEPTIONS)
		...
		else if (parameters.startsWith("CWithdrawF"))
		{
			if (Config.ALLOW_FREIGHT)
			{
				player.sendPacket(ActionFailed.STATIC_PACKET);
		...
		else if (parameters.startsWith("CDepositF"))
		{
			if (Config.ALLOW_FREIGHT)
			{
				if (player.getClan() == null)
				...
				if (player.getClan().getClanMembersForFreight().size() == 0)
				{
					player.sendPacket(new SystemMessage(SystemMessageId.CHARACTER_DOES_NOT_EXIST));
				}
				else
				{
					...
					Map<Integer, String> members = player.getClan().getClanMembersForFreight();
					...
					player.sendPacket(new PackageToList(members));
					player.setClanFreight(true);
				}
			}
		}
		...
		{
			if (item == null || item.getCount() < Config.CLEARPKCOUNT)
			{
				player.sendMessage("You don't have enough " + currencyName + ".");
				player.sendPacket(new ExShowScreenMessage("You don't have enough " + currencyName + ".", 2000, 2, false));
				player.sendPacket(new PlaySound("ItemSound3.sys_impossible"));
			...
			player.destroyItem("Consume", item.getObjectId(), Config.CLEARPKCOUNT, null, true);
		...
		else if (parameters.startsWith("delevel"))
		{
			...
			int lvl1 = playerLvl - Integer.parseInt(delevelNumber) + 1;
		...
		else if (parameters.startsWith("changename"))
		{
			...
			player.destroyItem("Consume", item.getObjectId(), 100, null, true);
		...
		else if (parameters.startsWith("SkillList"))
		{
			final int id = player.getClassId().getId();
			player.setTempAccess(true);
			if (id != 0)
			{
				player.setSkillLearningClassId(ClassId.values()[id]);
				showSkillList(player, ClassId.values()[id]);
			}
		}
		else if (parameters.startsWith("EnchantSkillList"))
		{
			player.setTempAccess(true);
			player.setCustomEnchantSkill(false);
			showEnchantSkillList(player);
		}
		else if (parameters.startsWith("CustomEnchantSkillList"))
		{
			player.setTempAccess(true);
			player.setCustomEnchantSkill(true);
			showEnchantSkillList(player);
		}
		else if (parameters.startsWith("observe"))
		{
			...
			doObserve(player, parameters.substring(7));
		}
		...
		else if (parameters.startsWith("resetOlyPoints"))
		{
			if (item == null || item.getCount() < 75)
			{
				player.sendMessage("You don't have enough " + currencyName + ".");
				player.sendPacket(new ExShowScreenMessage("You don't have enough " + currencyName + ".", 2000, 2, false));
				player.sendPacket(new PlaySound("ItemSound3.sys_impossible"));
			...
			player.destroyItem("Consume", item.getObjectId(), 75, null, true);
			OlympiadManager.getInstance().resetOlyPoints(player);
		}
		else if (parameters.startsWith("tvt"))
		{
			TvT.showEventHtml(player, String.valueOf(1));
		}
		else if (parameters.startsWith("ctf"))
		{
			DM.showEventHtml(player, String.valueOf(1));
		}
		else if (parameters.startsWith("dm"))
		{
			CTF.showEventHtml(player, String.valueOf(1));
		}
		else if (parameters.startsWith("vote"))
		{
			Vote.showHtm(player);
		}
	}
	...
	{
		String file = HtmCache.getInstance().getHtm("data/html/gmshop/item_enchanter/index.htm");
		NpcHtmlMessage html = new NpcHtmlMessage(1);
		file = file.replace("%maxenchant%", String.valueOf(PowerPackConfig.GMSHOP_ENCHNANT_MAX));
		html.setHtml(file);
		player.sendPacket(html);
	}
	
	public void enchantItem(L2PcInstance player, int objectId, int page)
	{
		...
		{
			L2ItemInstance itemEnchant = player.getInventory().getItemByObjectId(objectId);
			
			if (itemEnchant.getEnchantLevel() > PowerPackConfig.GMSHOP_ENCHNANT_MAX)
			{
				return;
			}
	...
	public void enchantMenu(L2PcInstance player, int page)
	{
		NpcHtmlMessage html = new NpcHtmlMessage(1);
		html.setFile("data/html/gmshop/item_enchanter/itemList.htm");
		...
		{
			if (item != null && item.isEquipped() && item.getItem().isCrystallizable() && item.getItem().getCrystalType() != L2Item.CRYSTAL_NONE && !item.isHeroItem())
			{
				if (item.getEnchantLevel() >= PowerPackConfig.GMSHOP_ENCHNANT_MAX)
				{
					list.remove(item);
				}
		...
		if (!list.isEmpty())
		{
			...
			for (L2ItemInstance item : list)
			{
				...
				String enchantLevel = String.valueOf(item.getEnchantLevel());
				...
				sb.append("<tr>");
				sb.append("<td width=40><button action=\"bypass custom_gmshop item_enchanter " + page + " " + item.getObjectId() + "\" width=32 height=32 back=" + L2Item.getItemIcon(itemId) + " fore=" + L2Item.getItemIcon(itemId) + "></td>");
				sb.append("<td width=220>");
				sb.append("<table width=300>");
				sb.append("<tr>");
				sb.append("<td><font color=799BB0>+" + enchantLevel + " " + name + "</font></td>");
				sb.append("<td></td>");
				sb.append("</tr>");
				sb.append("<tr>");
				sb.append("<td><font color=LEVEL>" + L2Item.getItemNameById(PowerPackConfig.GMSHOP_ENCHNANT_ITEM_ID) + "</font> (<font color=LEVEL>" + (PowerPackConfig.GMSHOP_ENCHNANT_ITEM_ID_COUNT) + "</font>)</td>");
				sb.append("</tr>");
		...
		sb.append("<td align=center width=70>" + (page > 1 ? "<button value=\"< Prev\" action=\"bypass custom_gmshop item_enchanter_page " + (page - 1) + "\" width=65 height=19 back=L2UI_ch3.smallbutton2_over fore=L2UI_ch3.smallbutton2>" : "") + "</td>");
		sb.append("<td align=center width=140>Page: " + page + " / " + pages + "</td>");
		sb.append("<td align=center width=70>" + (hasMore ? "<button value=\"Next >\" action=\"bypass custom_gmshop item_enchanter_page " + (page + 1) + "\" width=65 height=19 back=L2UI_ch3.smallbutton2_over fore=L2UI_ch3.smallbutton2>" : "") + "</td>");
		sb.append("</tr></table>");
	...
	public static void showSkillList(final L2PcInstance player, final ClassId classId)
	{
		// final int npcId = getTemplate().getNpcId();
		
		/*
		 * if (_classesToTeach == null) { if (player.isGM()) { NpcHtmlMessage html = new NpcHtmlMessage(getObjectId()); TextBuilder sb = new TextBuilder(); sb.append("<html><body>");
		 * sb.append("I cannot teach you. My class list is empty.<br> Ask admin to fix it. Need add my npcid and classes to skill_learn.sql.<br>NpcId:" + npcId + ", Your classId:" + player.getClassId().getId() + "<br>"); sb.append("</body></html>"); html.setHtml(sb.toString());
		 * player.sendPacket(html); return; } player.sendMessage("Error learning skills, contact the admin."); return; } if (!getTemplate().canTeach(classId)) { NpcHtmlMessage html = new NpcHtmlMessage(getObjectId()); TextBuilder sb = new TextBuilder(); sb.append("<html><body>");
		 * sb.append("I cannot teach you any skills.<br> You must find your current class teachers."); sb.append("</body></html>"); html.setHtml(sb.toString()); player.sendPacket(html); return; }
		 */
	...
	private void showBuyWindow(L2PcInstance player, int buyListId)
	{
		double taxRate = 0;
		player.tempInvetoryDisable();
		
		L2TradeList list = TradeController.getInstance().getBuyList(buyListId);
		
		if (list != null && list.getNpcId().equals(String.valueOf("shop")) || buyListId == 23 || buyListId == 500)
		{
		...
		{
			player.sendMessage("This list does not exist");
			LOG.warn(getClass().getSimpleName() + " possible client hacker: " + player.getName() + " attempting to buy from shop - buylist id:" + buyListId);
		}
	...
	private void doObserve(L2PcInstance player, String val)
	{
		...
		if (player.reduceAdena("Broadcast", cost, player, true))
		{
			// enter mode
