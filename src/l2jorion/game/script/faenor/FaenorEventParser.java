...
public class FaenorEventParser extends FaenorParser
{
	...
	public void parseScript(Node eventNode, ScriptContext context)
	{
		...
		{
			LOG.info(getClass().getSimpleName() + ": " + ID + " has passed... Ignored");
			return;
		}
		
		if (_eventDates.getStartDate().after(currentDate))
		{
			LOG.info(getClass().getSimpleName() + ": " + ID + " is not active yet... Ignored");
		...
		LOG.info(getClass().getSimpleName() + ": Loaded " + ID + " successfully");
