...
public final class Formulas
{
	...
	static class FuncAtkEvasion extends Func
	{
		...
		public void calc(Env env)
		{
			...
			{
				env.value += Math.sqrt(env.player.getDEX()) * 3;
				env.value += level;
				if (level > 77)
				{
					env.value += (level - 77);
	...
	public static double calcBlowDamage(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
	...
	public final static double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, boolean ss, boolean bss, boolean mcrit)
	{
		...
		// Failure calculation
		if (Config.ALT_GAME_MAGICFAILURES && !calcMagicSuccess(attacker, target, skill) && attacker instanceof L2PcInstance)
		{
			if (attacker instanceof L2PcInstance)
			{
	 ...
	 * @param attacker
	 * @param target
	 * @return true if shield defense successful
	 */
	...
	public boolean calcCubicSkillSuccess(final L2CubicInstance attacker, final L2Character target, L2Skill skill)
	{
		...
		// Resist modifier.
		final double vulnModifier = calcSkillVulnerability(target, skill, type);
		// lvl bonus modifier.
		final double lvlModifier = calcLvlDependModifier(attacker.getOwner(), target, skill);
		double rate = ((baseRate * statModifier * vulnModifier) + lvlModifier);
		...
		{
			attacker.getOwner().sendMessage("Cubic's Skill: " + skill.getName() + " chance: " + formatChanceNumber(rate) + "%");
		}
	...
	public boolean calcSkillSuccess(L2Character attacker, L2Character target, L2Skill skill, boolean ss, boolean sps, boolean bss)
	{
		...
		final double mAtkModifier = mAtkModifier(attacker, target, skill, sps, bss);
		// Level modifier.
		final double lvlModifier = calcLvlDependModifier(attacker, target, skill);
		// Physics configuration file
		final double physicsMult = getChanceMultiplier(skill);
		
		double rate = ((baseRate * statModifier * vulnModifier * mAtkModifier * physicsMult) + lvlModifier);
	 ...
	 * @return damage points
	 */
	public static final double calcChargeSkillsDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss, int _numCharges)
	{
		if (attacker instanceof L2PcInstance)
		{
			L2PcInstance pcInst = (L2PcInstance) attacker;
