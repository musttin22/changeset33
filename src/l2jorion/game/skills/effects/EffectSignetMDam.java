...
public final class EffectSignetMDam extends L2Effect
{
	...
	public boolean onActionTime()
	{
		...
		sps = caster.checkSps();
		bss = caster.checkBss();
		final boolean srcInArena = (caster.isInsideZone(ZoneId.ZONE_PVP) && !caster.isInsideZone(ZoneId.ZONE_SIEGE));
		...
		if (!targets.isEmpty())
		{
			...
			{
				if (!L2Skill.checkForAreaOffensiveSkills(caster, target, getSkill(), srcInArena))
				{
					continue;
				}
