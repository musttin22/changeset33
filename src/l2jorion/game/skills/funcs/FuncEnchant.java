...
public class FuncEnchant extends Func
{
	...
	public void calc(final Env env)
	{
		...
		int enchantCustom = item.getEnchantLevel();
		...
		if (env.player != null && env.player instanceof L2PcInstance)
		{
			...
			{
				// boost item by olympiad enchant limit
				if (player.isHero())
				{
				...
				// boost for tattoo
				if (item.getItem().getItemId() == 494)
				{
					if (stat == Stats.POWER_ATTACK)
					{
						env.value += (20 * enchantCustom);
					}
					if (stat == Stats.POWER_ATTACK_SPEED)
					{
						env.value += (30 * enchantCustom);
					}
					return;
				}
				
				if (item.getItem().getItemId() == 495)
				{
