		...
		public void run()
		{
			try
			{
				...
				switch (_timer)
				{
					...
					case 2:
						Broadcast.toAllOnlinePlayers("PvP zone will change in: " + _timer + " seconds", true);
						L2World.getInstance().getPlayers().stream().filter(player -> player.isInsideZone(ZoneId.ZONE_RANDOM)).forEach(player -> //
						player.sendPacket(new ExShowScreenMessage("PvP zone will change in: " + _timer + " seconds", 1000, 2, false)));
						break;
					case 1:
			...
			{
				LOG.error("RandomZoneTaskManager: Error in ZoneScheduler: " + e.getMessage(), e);
			}
		}
	}
