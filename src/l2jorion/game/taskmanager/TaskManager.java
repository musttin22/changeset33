...
import l2jorion.game.taskmanager.tasks.TaskDailyMissionsReset;
...
import l2jorion.game.taskmanager.tasks.TaskWeeklyBoard;
...
public final class TaskManager
{
	...
	private void initializate()
	{
		...
		registerTask(new TaskWeeklyBoard());
	...
	private boolean launchTask(ExecutedTask task)
	{
		...
		{
			long interval = Long.valueOf(task.getParams()[0]) * 86400000L; // 1 day
		...
		/*
		 * else if (type == TYPE_EXACT_TIME_TASK) { String[] hour = task.getParams()[1].split(":"); if (hour.length != 3) { LOG.warn("Task " + task.getId() + " has incorrect parameters"); return false; } Calendar calendar = Calendar.getInstance(); Calendar time = Calendar.getInstance(); Calendar
		 * time2 = Calendar.getInstance(); try { time.add(Calendar.DAY_OF_WEEK, Integer.valueOf(task.getParams()[0])); time.set(Calendar.DAY_OF_WEEK, Integer.valueOf(task.getParams()[0])); time.set(Calendar.HOUR_OF_DAY, Integer.valueOf(hour[0])); time.set(Calendar.MINUTE, Integer.valueOf(hour[1]));
		 * time.set(Calendar.SECOND, Integer.valueOf(hour[2])); if (calendar.get(Calendar.DAY_OF_WEEK) == Integer.valueOf(task.getParams()[0])) { time2.set(Calendar.DAY_OF_WEEK, Integer.valueOf(task.getParams()[0])); time2.set(Calendar.HOUR_OF_DAY, Integer.valueOf(hour[0]));
		 * time2.set(Calendar.MINUTE, Integer.valueOf(hour[1])); time2.set(Calendar.SECOND, Integer.valueOf(hour[2])); } } catch (Exception e) { if (Config.ENABLE_ALL_EXCEPTIONS) { e.printStackTrace(); } LOG.warn("Bad parameter on task " + task.getId() + ": " + e.getMessage()); return false; } long
		 * interval = time.getTimeInMillis() - System.currentTimeMillis(); long delay = time2.getTimeInMillis() - System.currentTimeMillis(); if (delay < 0) { delay += interval; } task.scheduled = scheduler.scheduleGeneralAtFixedRate(task, delay, interval); return true; }
		 */
